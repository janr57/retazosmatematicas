...
\section{Sistema de coordenadas cartesianas oblicuas}
...
componentes son idénticas, pero aquí sí hay diferencia.
Solo adelantaremos que las componentes contravariantes de expresan situando el índice
de la coordenada como superíndice (y no es un exponente), y la covariantes poniéndolo
como subíndice. Más adelante se aclararán estos términos.

\subsection{Características fundamentales del sistema}
\begin{itemize}

  
\item Ejes y su notación:
  Tendremos dos ejes. Uno de ellos, el horizontal, lo haremos coincidir con el eje $x$
  ...
  notaciones diferentes para los ejes:
  \begin{itemize}
  \item Notación en álgebra lineal y física clásica:
    ...
    En este texto, cuando utilicemos esta notación, siempre reservaremos $x$ e $y$ para
    el sistema cartesiano ortogonal, aunque el sistema oblicuo no lo comparemos con él.
  \item Notación en cálculo tensorial y geometría diferencial:
    Se usan preferentemente superíndices $x^1$ y $x^2$
    (que no representan exponentes, ni componentes de vectores, que también se pueden
    representar igual pero se distinguen por el contexto). Esto nos permitirá
    diferenciar entre componentes contravariantes (superíndices) y covariantes
    (subíndices)
  \end{itemize}
\end{itemize}
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Ejes oblicuos marcados como $\tilde{x}$ e $\tilde{y}$.
  Estos forman un ángulo $\alpha$ entre sí. Los ortogonales estándar $x$ e $y$ se
  representan por comparación. Nótese que se hace coincidir el eje $\tilde{x}$ con el
  $x$.}
  \label{fig:R2-oblic_ejes_xytilde}
\end{minipage}
\hspace{1em}
\begin{minipage}{0.45\linewidth}
  % Escala
  ...
  \begin{tikzpicture}[%
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Ejes oblicuos marcados como $x^1$ y $x^2$.
  Estos forman un ángulo $\alpha$ entre sí. Los ortogonales estándar $x$ e $y$ se
  representan por comparación. Nótese que se hace coincidir el eje $x^1$ con el
  $x$.}
  \label{fig:R2-oblic_ejes_x1x2}  
\end{minipage}
\end{figure}
...
Los ejes oblicuos los representamos por $\tilde{x}$ e $\tilde{y}$.
La base natural $B'$ en este sistema oblicuo está formada por los dos vectores
covariantes (nótese el superíndice)
\begin{equation}\label{eq:R2-oblic_base}
  B' = \set{\xhat{e}_1, \xhat{e}_2}
\end{equation}
La base natural se calcula expresando el vector $\vvv{r}=x\,\uvec{\i}+y\,\uvec{\j}$ en
función de las coordenadas en el nuevo sistema, en nuestro caso $x^1$ y $x^2$, y
calculando las derivadas parciales $\vvv{e}_i = \partial\vvv{r}/\partial x^i$.
En principio estos vectores no tienen por qué ser unitarios, aunque en este caso sí lo
serán; por esta razón les ponemos el acento circunflejo o \emph{gorrito}.

Un detalle más, hacemos coincidir los ejes $x$ y $\tilde{x}$.
