...
\section{Coordenadas cartesianas oblicuas}
Aquí introduciremos los conceptos de \emph{componentes contravariantes y covariantes},
algo que no hicimos en el sistema cartesiano rectangular porque en este último ambas componentes son idénticas, pero aquí sí hay diferencia.

\subsection{Base natural}
En coordenadas oblicuas es importante el ángulo $\alpha$ que forman los dos ejes.
Los ejes oblícuos los representamos por $\tilde{x}$ e $\tilde{y}$. La base natural está
formada por los vectores unitarios $\xhat{e}_1$ y $\xhat{e}_2$
...
\begin{figure}[ht]
  ...
  \caption{Sistema de coordenadas cartesianas oblicuas $\tilde{x}$ e $\tilde{y}$, que
    ...
    rojo los vectores unitarios $\xhat{e}_1$ y $\xhat{e}_2$ del sistema oblícuo y sus
...
base genérica $\set{\vvv{b}_1,\vvv{b}_2}$, escribiríamos
\[
...
la base $\set{\vvv{b}'_1, \vvv{b}'_2}$, las nuevas componentes serían $A'_i$ y $A'_2$
\[ 
  \vvv{v} = A'_1\vvv{b}'_1 + A'_2\vvv{b}'_2
\]
...
que las acompaña como \emph{superíndice} (que no representa un exponente). Así, un vector
como el que hemos puesto de ejemplo, se podría representar en forma genérica como
\[
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Componentes contravariantes $A^i$, en coordenadas oblicuas. El vector $\vvv{v}$
    se representa como $\vvv{v}=A^1\,\xhat{e}_1 + A^2\,\xhat{e}_2$. La componente
...
desplazamiento en $\symbb{R}^2$. La componente $A^1$ nos indica el espacio recorrido
en el eje $\tilde{x}$ y $A^2$ nos da el espacio recorrido en el eje $\tilde{y}$.
La suma de las componentes contravariantes $A^i$ multiplicadas por sus respectivos
vectores unidad, nos da el desplazamiento que nos indica el vector. 
...
\[
  \vvv{v}
  = A^1\,\xhat{u}_1 + A^2\,\xhat{u}_2 = A^i\,\xhat{u}_i
  = 2\,\uvec{\i} - 4\,\uvec{\j}
\]
...
Estas \emph{componentes covariantes} las representaremos mediante subíndices $A_i$
\begin{equation}
  \vvv{v} = A_1\xhat{e}^1 + A_2\xhat{e}^2 = A_i \xhat{e}^i
\end{equation}
La base utilizada aquí $\xhat{e}^i$, no es la natural de coordenadas oblicuas,
...
\begin{figure}[ht]
  \begin{minipage}{0.45\linewidth}  
    ...
    \begin{tikzpicture}[%
      ...
      \draw[covariante] (O) -- node[covtxt,pos=0.5, below] {$A_1\xhat{e}^1$} (COV1);
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Componentes covariantes $A_i$ en coordenadas oblicuas. El vector $\vvv{v}$
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      % Contravariantes
      \draw[proyeccion] (P) -- (CTV1);
      \node[orange!70!black, below=-1pt] at (CTV1) {\footnotesize $A^1$};
      \draw[proyeccion] (P) -- (CTV2);
      \node[orange!70!black, left=-1.5pt] at (CTV2) {\footnotesize $A^2$};
      % Covariantes
      \draw[proyeccion] (P) -- (COV1);
      \node[cyan!50!black, below=1pt] at (COV1) {\footnotesize $A_1$};
      \draw[proyeccion] (P) -- (COV2);
      \node[cyan!50!black, above left=-2pt and -2pt] at (COV2) {\footnotesize $A_2$};
  ...
  \end{minipage}
\end{figure}
Piensa en la base $\xhat{e}^i$ como una serie de líneas o superficies de nivel perpendiculares a cada uno de los ejes, que nos indican el valor de un cierto escalar o \emph{altura}. La componente covariante en ese eje nos indica de forma visual el
\emph{número de líneas o superficies de nivel} que el vector \emph{atraviesa}.
Ver figuras \ref{fig:R2-oblic_cov1} y \ref{fig:R2-oblic_cov2}.
\begin{figure}[ht]
  \begin{minipage}{0.45\linewidth}
    % Escala
    \def\scl{1.00}
    ...
    \begin{tikzpicture}[%
     ...
     covariante/.style={line width=1.8pt, cyan!70!black},
      ...
      %{$A_1\xhat{e}^1$} (COV1);
      \draw[covariante] (O) --node[covtxt,pos=0.5,below] {$A_1$} (COV1);
      %\draw[covariante] (O) -- node[covtxt,pos=0.5,above left=-2pt and -2pt]
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{En azul, la componente covariante $A_1$ de un vector $\vvv{v}$ en
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      \draw[covariante] (O)--node[covtxt,pos=0.5,above left=-2pt and -2pt]{$A_2$} (COV2);
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{En azul, la componente covariante $A_2$ de un vector $\vvv{v}$ en
      coordenadas oblicuas, interpretada geométricamente como la proyección del vector o
...
\subsection{Visión general de estas componentes}
Por si se quiere profundizar algo más en las componentes covariantes y contravariantes,
aquí se presentan algunas ideas útiles.
...
distintos.
\begin{itemize}
\item Por parte del álgebra lineal, la distinción entre componentes covariantes y
  contravariantes nace de la relación entre un espacio vectorial $V$,
  y su espacio dual $V^*$:
\begin{itemize}
...
Para terminar, el álgebra lineal proporciona la estructura algebráica, mientras que la geometría diferencial proporciona el marco geométrico y físico.
...
{\small
  \begin{align*}
    ...
    &=
      U^1\left(g_{11}\,V^1+g_{12}\,V^2\right)+U^2\left(g_{21}\,V^1+g_{22}\,V^2\right)\\
    &=
      U^1\left(\sum_{j=1}^2g_{1j}\,V^j\right) + U^2\left(\sum_{j=1}^2g_{2j}\,V^j\right)\\
    &=
      \sum_{i=1}^2 U^i \left(\sum_{j=1}^2\mmm{g}_{ij}\,V^j\right)\\
    &=
      \sum_{i=1}^2\sum_{j=1}^2\mmm{g}_{ij}\,U^i\,V^j
  \end{align*}
}
...
b) Nos proponemos realizar un cambio de base desde el sistema rectangular a otro oblícuo,
...
c) Obtén el tensor métrico en el sistema oblícuo anterior.\\
d) Halla las componentes contravariantes del vector.\\
e) ¿Cuáles son las componentes covariantes?\\
f) Halla el cuadrado del módulo del vector en el sistema oblícuo.\\
...
\begin{align*}
  \xhat{e}_1 &= \uvec{\i}
  \longrightarrow |\xhat{e}_1| = 1\\ 
  \xhat{e}_2 &= \cos\pi/3\,\uvec{\i} + \sin\pi/3\,\uvec{\j}
               = \dfrac{1}{2}\,\uvec{\i} + \dfrac{\sqrt{3}}{2}\,\uvec{\j}
               \longrightarrow |\xhat{e}_2| = \cos^2\pi/3 + \sin^2\pi/3 = 1
\end{align*}

La nueva base sería
\[
  \tilde{B} = \set{\xhat{e}_1, \xhat{e}_2}
...
\noindent
c) El tensor métrico en el sistema oblícuo, con $\alpha = \pi/3\,\unit{\radian}$ es
\[
  \mmm{g}_{ij}
  =\begin{pmatrix}
    g_{11} & g_{12}\\
...
de cambio de base
\[
  \left[A^i\right]_{\tilde{B}} = \mmm{M}^{-1}\, \left[A^i\right]_{B}
\]
donde las componentes contravarianes en la base rectangular $[A^i]_{B}$ son iguales
...
\[
  A_i = \mmm{g}_{ij}\,A^j
\]
Desarrollando la expresión para $A_1$ y $A_2$ y teniendo en cuenta la métrica,
$g_{11} = 1$, $g_{12}=g_{21}=1/2$ y $g_{22}=1$
\begin{align*}
  A_1
  &= g_{11}A^1 + g_{12}A^2 = A^1 + \dfrac{1}{2}A^2
  = 3 - \cancelout{\dfrac{4\sqrt{3}}{3}} + \cancelout{\dfrac{1}{2}\dfrac{8\sqrt{3}}{3}}
    = 3\\
  A_2
  &= g_{21}A^1 + g_{22} A^2 = \dfrac{1}{2}A^1 + A^2
...
En esta sección vamos a discutir un tipo de sistema de coordenadas oblicuo un tanto
peculiar. De hecho, no será un sistema de coordenadas en $\symbb{R}^2$, como hace
referencia el título del capítulo. Aun así, será un sistema de referencia en dos
dimensiones: espacio y tiempo.
En particular el espacio será unidimensional $\symbb{R}$, (una coordenada $x$), y además
tomaremos en consideración el tiempo $t$. Es decir, será un sistema con dos ejes $x$ y
$t$ y lo que mediremos no serán posiciones en $\symbb{R}^2$, sino \emph{sucesos}.

\subsection{Principio de relatividad de Galileo}
Empezaremos enunciando el principio de relatividad de Galileo y algunas suposiciones
que lleva implícitas.
...
\end{quote}
Esto significa que ningún experimento realizado en un sistema aislado puede
determinar si está quieto o moviéndose con velocidad constante.
...
Una transformación de Galileo es un cambio de coordenadas y velocidades que deja
invariante las ecuaciones de Newton.

Como ya avanzamos, nuestro espacio no será el de posiciones $\symbb{R}^2$, sino el de
\emph{sucesos}, tiempo, posición; así, nos centraremos en un eje temporal $t$ y otro
espacial $x$, prescindiendo de las coordenadas $y$ y $z$ para tener una transformación
de solo dos variables.

Para centrar ideas, supongamos que tenemos dos observadores, uno $O$ en reposo y el otro
$O'$ que lleva una velocidad constante $u$ con respecto al anterior.
El principio de Galileo permite relacionar las mediciones de tiempo y posición de un suceso, realizadas por ambos observadores $(t,x)$ y $(t',x')$. Aun así, representaremos
la posición en el eje de abcisas y el tiempo en ordenadas.
Las transformaciones de Galileo en una dimensión espacial $x$ son
\begin{subequations}
\begin{align}
  \label{eq:R2-transf_Galileo1}
  t' &= t\\
  \label{eq:R2-transf_Galileo2}
  x' &= x - ut
\end{align}
\end{subequations}
donde la segunda expresión indica que el tiempo es absoluto.

Ahora es importante hablar de los ejes de coordenadas, para tener una visualización
de estas transformaciones.
Empecemos diciendo que el observador $O$ utiliza un sistema de  referencia rectangular,
con ejes $t$ y $x$. Este observador puede registrar las posiciones y tiempos que observa
que tiene el segundo observador (recordemos que $O'$ se mueve con velocidad constante
con respecto a $O$); además, vamos a suponer que en el instante $t=0$, ambos se
encontraban en la misma posición $x'=0$ medida desde $O$. Como la velocidad es constante
la gráfica del movimiento de $O'$ respecto de $O$ es una línea de pendiente constante
$\alpha = \arccot(t/x)$. De la figura \ref{fig:R2-oblic_OO'_u}, se deduce que la velocidad relativa de $O'$ respecto de $O$ es
\begin{equation}
...
\footnotetext{Si hubiéramos representado el tiempo en abcisas y la posición en
  ordenadas, a mayor pendiente la velocidad sería mayor.}

En la figura \ref{fig:R2-oblic_suceso_transf2}, utilizamos lo visto en la anterior,
centrando nuestra atención en un suceso $S$ que tiene lugar en un cierto tiempo $t$, que
es observado por tanto por $O$, como por $O'$. En ella se comprueba que la distancia
$x'$ que mide $O'$ está de acuerdo con las transformación de Galileo
\eqref{eq:R2-transf_Galileo2}.
\begin{figure}[ht]
  \begin{minipage}{0.45\linewidth}
    % Escala
    \def\scl{1.00}
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      % Texto informativo
      \node[above right=20pt and 30pt] at (P) {$\left[x' = x - ut\right]$};
    ...
    \caption{Comprobación de una de las transformaciones de Galileo, la que relaciona
      la posición de un cierto suceso medido por $O$ y por $O'$ en un instante $t$.
      Aunque está implícito en la figura, no se impone con claridad que el tiempo es
      absoluto.}
    \label{fig:R2-oblic_suceso_transf2}
  \end{minipage}
\end{figure}

En forma matricial tendríamos\footnotemark{}
\footnotetext{En relatividad especial (Einstein), es costumbre cambiar el orden y
  ...
  el índice cero. Aquí, seguiremos este criterio, no porque sea necesario, sino porque
  es el estándar que seguiremos, para después adaptarnos a la relatividad einsteniana.}
\[
  \begin{pmatrix}
    t'\\
    x'
...
Calculando la inversa de la anterior tendremos la matriz de cambio de base
\begin{equation}
  \mmm{M}
  =
  \begin{pmatrix}
...
\subsection{Bases y matriz de cambio de base}
...
Nótese que, desde el punto de vista de $O'$, supuesto en reposo, el eje del tiempo de
$O$ se inclinaría en sentido antihorario formando un ángulo de $\pi/2+\alpha$.
Lo importante es que recordemos que lo que comentamos es desde el punto de vista de $O$.

\subsubsection{Base $B'$ para el observador $O'$ desde el punto de vista de $O$}
La base utilizada por $O'$, sería
\begin{equation}
  B' = \set{\xtilde{e}_0, \xtilde{e}_1}
  = \set{\xtilde{e}_t, \xhat{e}_x}
\end{equation}

En gran parte de nuestro desarrollo utilizaremos los índices numéricos $0$ y $1$ para
referirnos a los vectores de las bases y a las componentes según ellos, excepto cuando
nos interese referirnos a los valores de $x$, $t$, $x'$ y $t'$.
...
horizontales son líneas de igual tiempo $t=t'$, el vector temporal de la base $B'$ tiene
que ser mayor que el vector de la base temporal de $O$, que vale 1
\[
  \cos(\pi/2 - \alpha) = \sin\alpha = \dfrac{|\xhat{e}_0|}{|\xtilde{e}_0|};\hspace{0.5em}
  |\xtilde{e}_0| = \dfrac{1}{\sin\alpha}
\]

\begin{subequations}
\begin{align}
  |\xtilde{e}_0| &= |\tilde{e}_t| = \dfrac{1}{\sin\alpha}\\
  |\xtilde{e}_1| &= |\xhat{e}_1| = 1
\end{align}
\end{subequations}
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      \draw[versor, gray] (O) -- node[gray, pos=0.6, left]
        {\scriptsize $\xhat{e}_t$} ++(0, \EXTILDE cm) coordinate (e0fin);
      % Versor etilde0
      \draw[versor] (O) -- node[\colorTred, pos=0.55, right] {\scriptsize $\xtilde{e}_0$}
      ++(\YTILDEANG:\ETTILDE cm) coordinate (etilde0fin);
      % Versor etilde1
      \draw[versor] (O) -- node[\colorTred, pos=0.6, below]
      {\scriptsize $\xhat{e}_1 = \color{black!50}{\uvec{\i}}$}
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{En rojo, los vectores de la base $B' = \set{\xtilde{e}_0, \xtilde{e}_1}$
      deformados desde el punto de vista del observador en reposo $O$. En gris, el
      vector unitario temporal de la base $B$. Las líneas horizontales son
      equitemporales. La que se muestra equivale a \qty{1}{\second}.}
    % \label{fig:R2-oblic_contrav_cov}
  \end{minipage}
  \hspace{1em}
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      {\footnotesize $|\xtilde{e}_0| \cos\alpha$} (compx);
      %\node[below right=0pt and -4pt] at (compx)
      %{\footnotesize $|\xtilde{e}_0| \cos\alpha$};
      % Texto componente y
      \node[rotate=90, above left=18pt and 0pt] at (compy)
      {\footnotesize $|\xtilde{e}_0| \sin\alpha$};      
      ...
      % Versor etilde0
      \draw[versor] (O) -- node[\colorTred, pos=0.65, left] {\scriptsize $\xtilde{e}_0$}
      ++(\YTILDEANG:\ETTILDE cm) coordinate (etilde0fin);
      % Versor etilde1
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Componentes horizontales y espaciales en la base $B$ del observador en reposo
      $O$ del vector temporal de la base $B'$. Como su módulo es
      $|\xtilde{e}_0|=1/\sin\alpha$, se deduce que
      $\xtilde{e}_0 = \cot\alpha\,\uvec{i} + \xhat{e}_t$ en la base $B$ del observador
      en reposo $O$.}
    % \label{fig:R2-oblic_contrav_cov}
  \end{minipage}
\end{figure}
...
%  \begin{equation}
%    \mmm{g}_{ij} = \begin{pmatrix}1 & 0\\ 0 & 1/r^2 \end{pmatrix}
%  \end{equation}
