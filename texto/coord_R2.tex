...
En particular el espacio se limitará a $\symbb{R}$ (una coordenada $x$), y además
...
invariante las ecuaciones de Newton. Como avanzamos atrás, en esta sección nos
centraremos en un eje espacial $x$ y el tiempo $t$.

Para centrar ideas, supongamos que tenemos un observador $O$ en reposo y otro
$O'$ que lleva una velocidad constante $u$ con respecto del anterior.
El principio de Galileo permite relacionar las mediciones de posición y tiempo
de un objeto, realizadas por ambos observadores $(x,t)$ y $(x',t')$
\begin{align*}
  x' &= x - ut\\
  t' &= t
\end{align*}
...
\footnotetext{En relatividad especial (Einstein), es costumbre cambiar el orden y
  ...
  el índice cero. Aquí, por sencillez, no seguiremos ese convenio.}
\[
  \begin{pmatrix}
    x'\\
    t'
...
Estas componentes son contravariantes, luego podremos escribir $X^1 = x$, $X^2 = t$, $\tilde{X}^1 = x'$ y $\tilde{X}^2 = t'$. Las transformaciones de Galileo quedarían
\begin{equation}
