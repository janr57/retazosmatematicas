...
\begin{align}
  \xtilde{e}_0
  &= \xtilde{e}_{t'} = \xhat{e}_t + \cot\alpha\,\uvec{\i} = (1, \cot\alpha)\\
  \xhat{e}_1
  &= \xhat{e}_{x'} = 0\xhat{e}_t+\xhat{e}_x = 0\xhat{e}_t + \uvec{\i} = (0, 1)
\end{align}

La matriz de cambio de base se construye situando las componentes de los vectores de la
base $B'$ en función de $B$ en columnas. Así $B' = B \mmm{M}$
\begin{equation}
  \begin{pmatrix}
...
La inversa de la métrica sería
\begin{align*}
  \mmm{g}^{ij}
  &=
...
en $O'$ desde el punto de vista de $O$. En notación de Einstein
\[
  \vvv{v} = X^i\,\tilde{e}_i
