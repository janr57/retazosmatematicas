...
\subsection{Introducción y base natural}
...
\subsubsection{Transformación de vectores. Cambio de base}
...
\subsubsection{Transformación de componentes de vectores. Componentes
  contravariantes}
Este apartado es importante y conviene presentarlo con cierto detalle.
...
\subsubsection{Forma matricial de la base y de las componentes}
En forma matricial, las componentes del vector $\vvv{v}$ en una cierta base se
representan en columna. Así, el vector anterior se puede expresar en la base elegida como
\[
  \vvv{v} =
  \begin{pmatrix}
    A^1\\
  ...
  \end{pmatrix}
\]
Recordemos que los vectores de una base lo hacen situando los vectores en fila
\[
  B =
  \begin{pmatrix}
    \vvv{b}_1 & \vvv{b}_2
...
común es que son una representación matricial, pero de cosas diferentes.
...
\subsubsection{Transformación de componentes}
Las componentes de un vector se representan en columna y en la expresión matemática
interviene la matriz inversa (las componentes contravariantes se transforman de forma
inversa a como lo hacían los vectores de la base). Así, las nuevas componentes se
obtienen a partir de las antiguas mediante $\mmm{M}^{-1}$, mientras que las antiguas
se transforman a partir de las nuevas mediante la inversa de la anterior, esto es
$\mmm{V}$
\begin{subequations}
  \begin{align}
    \begin{pmatrix}
...
Los vectores se transforman de forma inversa a la base. Así, si tenemos un vector en
coordenadas rectangulares $\vvv{u} = 2\,\uvec{i} - 4\,\uvec{j}$, entonces, para hallar
las componentes en un sistema de coordenadas oblícuas, con
$\alpha = \pi/3\unit{\radian}$, tenemos que aplicar la inversa de $M$
{\small
\begin{align*}
  \begin{pmatrix}
    u^1\\
  ...
  &=
    ...
    \begin{pmatrix}
      2\\
      \dfrac{2\cos(\pi/3)-4}{\sin(\pi/3)}
    \end{pmatrix}
    =
    \begin{pmatrix}
      2\\
      \dfrac{2\dfrac{1}{2}\dfrac{\sqrt{3}}{2}-4}{1/2}
    \end{pmatrix}
\end{align*}
}
...
\subsection{Producto escalar de los vectores de la base}
...
%    % \node[above] at (P) {$P$};
    ...
    \end{scope}
  \end{tikzpicture}
  % \caption {Componentes contravariantes.}
  % \end{minipage}
  \hspace{0.5em}
  % \begin{minipage}{0.40\linewidth}
  %   \centering
  %   \caption {Componentes covariantes.}
  % \end{minipage}
