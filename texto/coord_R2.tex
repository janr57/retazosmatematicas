...
\begin{itemize}
\item Los elementos diagonales valen la unidad. Esto significa que un cambio en cualquiera
  de las coordenadas $\tilde{x}$ o $\tilde{y}$, representan ese mismo valor de
  desplazamiento en el espacio.
\item Los elementos no diagonales no son cero, lo que indica que un cambio cualquiera en
  las coordenadas implica un cambio en la otra, y viceversa (los ejes no son ortogonales).
\end{itemize}
...
paralelogramo---. Los resultados de esas proyecciones paralelas eran las componentes
contravariantes del vector.
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
  % 
  ...
  \caption{Componentes covariantes $A_i$ en coordenadas oblícuas. El vector $\vvv{v}$
    ...
    contravariantes nos indican el desplazamiento en el espacio. Nótese que la base
    utilizada no es la natural.}
...
\begin{figure}[ht]
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{La componente covariante $A_1$ de un vector $\vvv{v}$ en coordenadas
      oblícuas, en azul, y se puede interpretar geométricamente como la proyección del
      vector o sombra en el eje $x$.}
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Componente covariante $A_2$ del vector en coordenadas oblícuas.}
...
%    \pgfmathsetmacro{\XNUMLINEAS}{\XNUMLINEASMASUNA - 1}
%    \pgfmathsetmacro{\XUNIT}{\XTILDELONG / \XNUMLINEASMASUNA}
%    \pgfmathsetmacro{\ALTURALINEAS}{\YTILDELONG * cos(90-\YTILDEANG) - \INICIOALTURA}
%      % Lineas de nivel y
%      \pgfmathsetmacro{\YNUMLINEAS}{4}
%      \pgfmathsetmacro{\YUNIT}{\YTILDELONG / (\YNUMLINEAS + 1)}
%    % Fondo
%    \pgfmathsetmacro{\HORZ}{0.25}
%    \pgfmathsetmacro{\VERT}{0.25}
%    % 
...
%    \pgfmathsetmacro{\YNUMLINEAS}{\YNUMLINEASMASUNA - 1}
%    \pgfmathsetmacro{\YUNIT}{\XTILDELONG / \YNUMLINEAS}
%    \pgfmathsetmacro{\LONGITUDLINEAS}{\XTILDELONG - \INICIOLONGITUD}
%    % Fondo
%    \pgfmathsetmacro{\HORZ}{0.25}
%    \pgfmathsetmacro{\VERT}{0.25}
%    % 
...
\begin{equation}
  \vvv{v} = A_1\vvv{e}^1 + A_2\vvv{e}^2 = A_i \vvv{e}^i
\end{equation}
La base utilizada aquí $\vvv{e}^i$, no es la natural de coordenadas oblícuas,
\eqref{eq:R2-oblic_base}, pues contiene un superíndice (no es un exponente).
Más adelante se dará una explicación sencilla.
...
Hay una definición más formal de las componentes covariantes, que procede del álgebra
lineal y de la geometría diferencial. Esto implica la introducción de los conceptos de
\emph{espacio dual} y \emph{1-forma} (o \emph{covector}). Es como si el espacio (en
nuestro caso $\symbb{R}^2$) no contuviera solo segmentos orientados (vectores normales),
cuya base es la B' en \eqref{eq:R2-oblic_base}, sino que también contiene \emph{superficies de nivel} o \emph{gradientes} ---en nuestro ejemplo en dos dimensiones,
más que superficies de nivel serían líneas de nivel---.
...
Hay varias formas de introducir estas componentes. Por un lado tenemos la explicación
geométrica y por otro una explicación más abstracta.

Empecemos por la explicación geométrica...
...
%    
    ...
    % Contravariantes
    \draw[proyeccion] (P) -- (CTV1);
    \node[below] at (CTV1) {\footnotesize $A^1$};
    \draw[proyeccion] (P) -- (CTV2);
    \node[left] at (CTV2) {\footnotesize $A^2$};
    % Covariantes
    \draw[proyeccion] (P) -- (COV1);
    \node[below] at (COV1) {\footnotesize $A_1$};
    \draw[proyeccion] (P) -- (COV2);
    \node[above left=-2pt and -2pt] at (COV2) {\footnotesize $A_2$};
