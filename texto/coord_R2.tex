...
Cuando vimos las componentes contravariantes de un vector en la sección
\ref{ssect:R2-contravariantes}, lo proyectamos trazando líneas paralelas a los ejes. Así
es como sumamos vectores gráficamente, mediante la regla del paralelogramo.
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
  % 
...
En cambio, hay otra forma de proyectar el vector, y es trazando líneas perpendiculares a
los ejes, como se aprecia en la figura \ref{fig:R2-oblic_cov}.
Estas \emph{componentes covariantes} las representaremos mediante subíndices $A_i$.
Mientras que las contravariantes te dicen cuánto \emph{avanzas} a lo largo de cada eje,
las covariantes miden la \emph{sombra} o proyección sobre ese eje.

Hay una definición más formal de las componentes covariantes, que procede de la geometría
diferencial. Esto implica la introducción del concepto de \emph{1-forma} o
\emph{covector}. Es como si el espacio (en nuestro caso $\symbb{R}^2$) no contuviera solo
segmentos orientados (vectores normales), sino que también contiene
\emph{superficies de nivel} o \emph{gradientes}.
  
Una forma de asimilar este tipo de componentes es suponer que un vector $\vvv{v}$
representa el gradiente de un campo escalar $\vvv{\nabla}\phi$.
Si aumentamos la escala de medida (utilizamos una base \emph{mayor}), las superficies
de nivel (gradiente) se comprimen y, aunque el vector se ve más pequeño (componentes
contravariantes), la densidad de líneas de nivel aumenta (componentes covariantes).
