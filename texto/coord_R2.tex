...
\item La primera es, ¿cuál de los ejes utilizo como abcisas y cual como ordenadas?
  ...
  quiero utilizar el esquema que se utiliza en \emph{relatividad especial}, aunque no voy a utilizar el eje
  del tiempo multiplicado por la velocidad de propagación de la luz en el vacío $c$ como
  se hace en esta última.
\item La otra es, qué angulo utilizar para el desarrollo matemático que seguirá.
  Teniendo en cuenta que esta línea del movimiento relativo de $O'$ respecto de $O$,
  se convertirá en el eje $\tilde{t}$ de $O'$ desde el punto de vista de $O$.
  ...
  \item Utilizar el ángulo $\alpha$ que forma el eje $x$ (abcisas) con la línea de
    ...
    observador $O$, en reposo, el eje $\tilde{t}$ que utiliza $O'$ como referencia. El
    observador en reposo siente que este último no utiliza ejes ortogonales, sino que
    forman un ángulo distinto $\alpha$, en este caso menor de \qty{90}{\degree}, porque
    $O'$ se aleja de $O$ en el sentido positivo del eje $x$ (sería mayor de
    \qty{90}{\degree} si se alejara en sentido contrario.
    \itme Utilizar el ángulo complementario $\theta = 90-\alpha\,\unit{\degree}$, que
    forma la línea del movimiento de $O'$ con respecto del eje $t$.
  \end{enumerate}
\end{enumerate}

la otra decisión es, una vez decido por una de las anteriores, qué angulo utilizar para
el desarrollo matemático, esto es, 
utilizar el ángulo $\alpha$ que forma la línea con el eje $x$ o su complementario, el que
forma el eje $t$ con la línea recta. Cualquiera de las dos opciones sería correcta, pero
me he dedicido a utilizar el ángulo citado por varias razones:
\begin{itemize}
\item Este ángulo nos va a indicar cómo ve el observador $O$ en reposo los ejes que
  utiliza $O'$ como referencia. El observador en reposo siente que este último no utiliza
  ejes ortogonales, sino que forman un ángulo $\alpha$.
\item El ángulo complementario, que voy a llamar $\theta = 90-\alpha$ en este comentario,
  se puede interpretar (figura \ref{fig:R2-oblic_etprima_et}) como el ángulo que el
  observador en reposo, ve con que \emph{cierra} el eje $t'$ sobre $x'$ del observador
  $O'$ en movimiento. Desde mi punto de vista, este ángulo, debería ser $-\theta$, pues
  se cierra en sentido horario. Aunque no es necesario tener en cuenta el signo
  negativo, me siento un poco incómodo con esto. En todo caso, $\theta$ sería la
  verdadera pendiente $x/t$ de la línea recta.
\item Por otro lado, podría haber utilizado el eje $t$ como el de abcisas y el $x$ como
  el de ordenadas, que es lo más normal en física elemental. Pero quiero acostumbrarme
  al estilo en relatividad especial de situar el eje $t$ en ordenadas, aunque en la
  relatividad de Galileo no es necesario hacerlo, y puede parecer algo artificial.
\end{itemize}

, vemos que
$\alpha = \arccot(x/t)$ y que la velocidad relativa de $O'$ respecto de $O$ es
\begin{equation}
...
\footnotetext{Si hubiéramos representado el tiempo en abcisas y la posición en
  ordenadas, a mayor velocidad le correspondería una pendiente mayor.}
...
\subsection{Componentes covariantes y contravariantes}
Bajar índices
$X_t = g_{tt}X^{t} + g_{tx} X^{x}= X^{t} + u X^{x}$
