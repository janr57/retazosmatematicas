...
\subsubsection{En función de las componentes contravariantes}
Supongamos que tenemos un campo escalar $\phi$ definido en $\symbb{R^2}$ y expresado en
coordenadas oblicuas, según sus componentes contravariantes ---utilizando, para la
posición en el espacio, la base unitaria $B= (\xhat{e}_1,\xhat{e}_2)$---
\begin{equation}
  \phi = \phi(x^1, x^2)
\end{equation}
Como sabemos, los vectores de esta base no cambian con la posición en el espacio.
Por tanto, Para obtener el gradiente con sus componentes contravariantes $\nabla\phi^1$
y $\nabla\phi^2$, tenemos que derivar parcialmente el campo con respecto a $x^i$, pero
como este resultado es un vector covariante (1-forma)
...
\begin{equation}
  ...
  = \mmm{g}^{ij} \dfrac{\partial\phi}{\partial x^i}\,\xhat{e}_j
...
{\small
Nótese que también se podrían haber calculado las componentes por separado
\begin{equation}
  (\nabla\phi)^i
  = \mmm{g}^{ij}\dfrac{\partial\phi}{\partial x^j}
\end{equation}
}
...
En la figura, vemos que la velocidad relativa de $O'$ respecto de $O$ es la cotangente
de $\alpha$
\begin{equation}
...
Vamos a subir los índices de los vectores de $B$, $\xtilde{e}_i = \mmm{g}^{ij}\xtilde{e}_j$
En la base $B'$
\begin{align*}
  \xtilde{e}^0
  &=
