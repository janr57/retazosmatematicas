...
\begin{subequations}
  \begin{align}
    ...
    \left[A^i\right]_{B} &= \mmm{M} \left[A^i\right]_{B'}
  \end{align}
\end{subequations}
...
representar gráficamente, ver figura \ref{fig:R2-oblcontrav}.
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
  % 
  ...
  \begin{tikzpicture}[%
    ...
    %textoapagado/.style={\colorTorig},
    vector/.style={-{Latex[round]}, shorten >=1.2pt, line width=.8pt,\colorV},
    contravariante/.style={line width=2pt, orange!90!black},
    %vtilde/.style={%
    %  -{Latex[round,width=4pt,length=6pt]}, line width=1pt, \colorVred},
    pcirculo/.style={fill=\colorPorig, draw=black},
    angulo/.style={fill=green!15, draw=\colorGirodos},
...
%    
    ...
    \draw[contravariante] (O) -- node[pos=0.5, below] {$\hat{A}^1$} (CTV1);
    \draw[contravariante] (O) -- node[pos=0.5, above left=-2pt and -4pt] {$\hat{A}^2$} (CTV2);
...
%    % \node[above] at (P) {$P$};
    ...
    \end{scope}
  \end{tikzpicture}
  \label{fig:R2-oblcontrav}
  \caption{Componentes contravariantes $A^i$ en coordenadas oblícuas. El vector $\vvv{v}$
    se representa como $\vvv{v}=A^1\,\xhat{e}_1 + A^2\,\xhat{e}_2$. No se representan los
    vectores de la base oblícua para no complicar la figura innecesariamente.
    Las componentes contravariantes indican el desplazamiento en el espacio
    $\symbb{R}^2$.}
\end{figure}
