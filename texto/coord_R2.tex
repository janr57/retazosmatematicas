...
En coordenadas oblícuas es importante el ángulo $\alpha$ que forman los dos ejes.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Sistema de coordenadas cartesianas oblícuas $\tilde{x}$ e $\tilde{y}$, que
...
En coordenadas oblícuas en $\symbb{R}^2$ utilizamos las matrices inversa
...
métrico en coordenadas oblícuas es utilizar el método seguido en la expresión
...
El tensor métrico en coordenadas oblícuas resulta
...
\end{equation}
La base utilizada aquí $\xhat{e}^i$, no es la natural de coordenadas oblícuas,
...
\begin{figure}[ht]
  \begin{minipage}{0.45\linewidth}
    ...
    \caption{En azul, la componente covariante $A_1$ de un vector $\vvv{v}$ en
      coordenadas oblícuas, interpretada geométricamente como la proyección del vector o
...
\noindent
f) El cuadrado del módulo del vector calculado con las componentes oblícuas debe ser
