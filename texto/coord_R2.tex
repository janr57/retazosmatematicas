...
Los ejes oblicuos los representamos por $\tilde{x}$ e $\tilde{y}$. La base natural está
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Componentes contravariantes $\tilde{x}^i$, en coordenadas oblicuas. El vector
    $\vvv{v}$ se representa como
    $\vvv{v}=\tilde{x}^1\,\xhat{e}_1+\tilde{x}^2\,\xhat{e}_2$. La componente
...
desplazamiento en $\symbb{R}^2$. La componente $\tilde{x}^1$ nos indica el desplazamiento
en el eje $\tilde{x}^1 = \tilde{x}$, y $\xtilde{x}^2$ nos lo da en el eje
$\tilde{x}^2=\tilde{y}$.
La suma de las componentes contravariantes $\tilde{x}^i$ multiplicadas por sus
respectivos vectores unidad, nos da el desplazamiento indicado por el vector.
...
Se propone como ejercicio demostrar que la métrica inversa es
{\small
...
\subsubsection{Base contravariante}
A continuación obtendremos una expresión de la base contravariante
$B'=(\vvv{e}^1,\vvv{e}^2)$ en función de la covariante $B=(\xhat{e}_1, \xhat{e}_2)$.
Nótese que esta nueva base no tiene por qué ser unitaria --por ello, no les hemos puesto
el acento circunflejo. Para obtenerlos, tenemos que \emph{subir} índices.
En notación de Einstein
\begin{equation}
  \vvv{e}^i = g^{ij}\xhat{e}_j
\end{equation}

Utilizamos la métrica inversa \eqref{eq:R2-oblic_metrica_inv}
\begin{align*}
  \vvv{e}^1
  &=
    g^{11}\xhat{e}_1 + g^{12}\xhat{e}_2
    =
    \dfrac{1}{\sin^2\alpha}\,\xhat{e}_1 - \dfrac{\cos\alpha}{\sin^2\alpha}\,\xhat{e}_2
    = \dfrac{1}{\sin^2\alpha} (\xhat{e}_1 - \cos\alpha\,\xhat{e}_2)\\
    \vvv{e}^2
  &=
    g^{21}\xhat{e}_1 + g^{22}\xhat{e}_2
    =
    -\dfrac{\cos\alpha}{\sin^2\alpha} \xhat{e}_1 + \dfrac{1}{\sin^2\alpha} \xhat{e}_2
    = \dfrac{1}{\sin^2\alpha} (-\cos\alpha\,\xhat{e}_1 + \xhat{e}_2)\\
\end{align*}

Comprobamos el módulo de estos vectores, pudiéndose comprobar fácilmente que solo serían
unitarios en un sistema cartesiano rectangular $\alpha=\qty{90}{\degree}$
\begin{align}
  \left|\vvv{e}_1\right|
...
e) Calcula estas componentes mediante transformación de los vectores de la base $B$.\\
f) Calcula los vectores de la base covariante $B'=\set{\vvv{e}_1, \vvv{e}_2}$ en función
de la base en coordenadas ortogonales y comprueba que son unitarios.\\
...
que no son vectoes unitarios\\
i) Calcula los productos escalares entre los elementos de la base covariante y la
contravariante\\
...
elementos $\omega$ del espacio dual de los vectores $\vvv{v}$ del espacio tangente de $\symbb{R}^2$ y que estas $\omega$ son aplicaciones lineales que aplicadas sobre los
vectores del espacio tangente, nos producen un escalar, y halla este escalar para nuestro
vector $\vvv{v}$.\\
...
\noindent
b) Matriz de cambio de base $B=\set{\uvec{\i},\,\uvec{\j}}$ a la de coordenadas oblicuas
que forman $\qty{60}{\degree}$, haciendo coincidir los ejes $x$ y $x'$. Calculando
también su inversa.

En la figura \ref{fig:R2-coord_oblicuas_e1e2} se puede observar que, en nuestro caso
\begin{align*}
  \vvv{e}_1
  &=
    \uvec{\i}\\
...
\begin{align*}
  ...
  |\uvec{e}_2|^2
  &=
    \left(\dfrac{1}{2}\,\uvec{\i}+\dfrac{\sqrt{3}}{2}\,\uvec{\j}\right)\cdot\uvec{\i}
    =1
\end{align*}
...
\noindent
c) La matriz de cambio de base $\mmm{M}$ se presentó en \ref{eq:R2-oblic_M}. Para
calcular las componentes en la nueva base, tenemos que utilizar la matriz inversa
$\mmm{M}^{-1}$, ver \ref{eq:R2-oblic_Minv}
\[
  [x]_{B'} = \mmm{M}^{-1} [x]_{B}
\]
\[
...
\noindent
d) Necesitamos obtener los vectores de la base antigua $B=\set{\uvec{\i},\uvec{\j}}$ en
función de la nueva $B'=\set{\vvv{e}_1,\vvv{e}_2}$. Para ello utilizaremos inversa de la
...
El vector expresado con componentes contravariantes es, en notación de Einstein
\[
  \vvv{v} = x^i\vvv{e}_i
\]
...
Las componentes contravariantes son, obviamente, las mismas que en el apartado anterior
\begin{align*}
  x^1 &= 1\\
  x^2 &= 2
\end{align*}
...
\noindent
e) Vectores de la base covariante $B'=\set{\vvv{e}_1,\vvv{e}_2}$ en función de la base
  en coordenadas cartesianas ortogonales $B=\set{\uvec{\i},\,\uvec{\j}}$.
...
Los vectores de la base natural resultan
\begin{align*}
  \vvv{e}_1
  &=
    \uvec{\i}\\
...
Estos vectores son unitarios $\vvv{e}_1 = \xhat{e}_1$ y $\vvv{e}_2 = \xhat{e}_2$
\begin{align*}
  |\vvv{e}_1|^2
  &=
    \uvec{\i}\cdot\uvec{\i}
...
\noindent
g)
En el apartado d) calculamos los vectores de la base covariante, resultando
\begin{align*}
  \vvv{e}_1 &= \uvec{\i}\\
...
\noindent
h) Productos escalares entre elementos de las bases covariante y contravariante
\begin{align*}
...
\noindent
i) Las componentes covariantes del vector se pueden calcular bajando el índice de las
contravariantes
\[
  x_i = g_{ij} x^j
\]
...
\noindent
i) El vector $\vvv{v}$ es un elemento del espacio tangente de $\symbb{R}^2$
\[
  \vvv{v} \in T_p\symbb{R}^2
\]

Podemos establecer una aplicación lineal $\omega$ que, aplicada a un vector produzca un
escalar
\[
  \omega: T_p\symbb{R}^2 \rightarrow \symbb{R}
\]
y diremos que $\omega$ es un elemento del \emph{espacio dual} del vector
\[
...
los vectores. La métrica permite relacionar los dos espacios (subir o bajar índices).

Ahora nos podemos preguntar que si
$\omega = x_1\vvv{e}^1 + x_2\vvv{e}^2 = 2\vvv{e}^1 + \frac{5}{2}\vvv{e}^2$ es una de
tales aplicaciones, debería \emph{comer} un vector como $\vvv{v}$ y producir un escalar
(en este caso un número real).
...
\begin{equation}
  \vvv{\nabla}\phi
  = (\nabla\phi)_i\xhat{e}^i
  = \dfrac{\partial\phi}{\partial x^i}\xhat{e}^i
  = \dfrac{\partial\phi}{\partial x^1}\xhat{e}^1
  + \dfrac{\partial\phi}{\partial x^2}\xhat{e}^2
\end{equation}

\subsubsection{Componentes contravariantes}
Para que el gradiente nos indique la dirección y sentido que indica el máximo aumento del
escalar tenemos que obtener sus componentes contravariantes (segmentos orientados en el
espacio).

Para obtener el gradiente con sus componentes contravariantes $\nabla\phi^1$ y
$\nabla\phi^2$, tenemos que derivar parcialmente el campo con respecto a $x^i$, pero como
este resultado es un vector covariante (1-forma)
$\nabla\phi_i=\partial\phi/\partial x^i$, tenemos que subir índices multiplicando por la
...
\subsubsection{Relación con las componentes cartesianas rectangulares}
Si conociéramos las componentes cartesianas rectangulares del gradiente
$(\partial\phi/\partial x, \partial\phi/\partial y)$, podríamos utilizar la matriz
inversa de cambio de base $\mmm{M}^{-1}$ \eqref{eq:R2-oblic_Minv}, para obtener las
componentes covariantes en oblicuas en función de las rectangulares
\begin{equation}
  \begin{pmatrix}
    \nabla\phi_1\\
    \nabla\phi_2
  ...
  \end{pmatrix}
  \begin{pmatrix}
    \dfrac{\partial\phi}{\partial x}\\[2ex]
    \dfrac{\partial\phi}{\partial y}
  \end{pmatrix}
  = \begin{pmatrix}
    \dfrac{\partial\phi}{\partial x}-\cot\alpha\,\dfrac{\partial\phi}{\partial y}\\[1ex]
    \csc\alpha\,\dfrac{\partial\phi}{\partial y}
  \end{pmatrix}
\end{equation}

Las componentes resultan
\begin{subequations}
  \begin{align}
    (\nabla\phi)_1
    &=
...
El gradiente se podrá representar como $\vvv{\nabla}\phi = (\nabla\phi)_i\,\vvv{e}^i$
\begin{equation}
  \vvv{\nabla}\phi
  = \left(%
    \dfrac{\partial\phi}{\partial x} - \cot\alpha\,\dfrac{\partial\phi}{\partial y}
  \right)\vvv{e}^1
  + \csc\alpha\,\dfrac{\partial\phi}{\partial y}\,\vvv{e}^2
\end{equation}

Las componentes contravariantes se pueden hallar subiendo índices
\begin{equation}
  (\nabla\phi)^i = g^{ij} (\nabla\phi)_j
\end{equation}
...
\begin{align*}
  ...
  &=
    ...
    \dfrac{\partial\phi}{\partial x}-\cot\alpha\,\dfrac{\partial\phi}{\partial y}
    \right)
    - \dfrac{\cos\alpha}{\sin^2\alpha}\csc\alpha\,\dfrac{\partial\phi}{\partial y}
\end{align*}
\begin{align*}
  \nabla\phi^2
  &=
  ...
  &=
    ...
    \dfrac{\partial\phi}{\partial x}-\cot\alpha\,\dfrac{\partial\phi}{\partial y}
    \right)
    + \dfrac{1}{\sin^2\alpha}\csc\alpha\,\dfrac{\partial\phi}{\partial y}
    \right]
\end{align*}

Simplificando las dos expresiones anteriores nos queda
\begin{subequations}
  \begin{align}
    \nabla\phi^1
    &=
      ...
      -2\cot\alpha\,\dfrac{\partial\phi}{\partial y}
      \right)\\
    \nabla\phi^2
    &=
      \dfrac{1}{\sin^2\alpha}
      ...
      + \dfrac{1+\cos^2\alpha}{\sin\alpha}\,\dfrac{\partial\phi}{\partial y}
      \right)  
  \end{align}
\end{subequations}

El gradiente se podrá escribir como
{
  \begin{equation}
    \vvv{\nabla}\phi
    =
  ...
  Obtener el gradiente de un campo escalar $\phi=\phi(x,y)$,  en coordenadas oblicuas con
  componentes contravariantes, a partir del gradiente en coordenadas cartesianas
  rectangulares
  \[
    \vvv{\nabla}\phi
    = \dfrac{\partial\phi}{\partial x}\,\uvec{\i}
...
\noindent
\begin{itemize}
\item Obtenemos los vectores $\uvec{\i}$ y $\uvec{\j}$ en función de los vectores
...
\item Calculamos la expresión de las derivadas parciales en coordenadas rectangulares
  a partir de las componentes contravariantes en oblicuas
  \[
    \begin{pmatrix}
      \dfrac{\partial\phi}{\partial x}\\[2ex]
      \dfrac{\partial\phi}{\partial y}
    ...
    \begin{pmatrix}
      \dfrac{\partial\phi}{\partial x^1}+\cos\alpha\,\dfrac{\partial\phi}{\partial x^2}\\
      \sin\alpha\,\dfrac{\partial\phi}{\partial x^2}
    \end{pmatrix}
  \]
...
Ahora sustituimos en la expresión del gradiente en rectangulares
\begin{align*}
  \dfrac{\partial\phi}{\partial x}\,\uvec{\i}
  + \dfrac{\partial\phi}{\partial y}\,\uvec{\j}
  &=
    ...
    + \sin\alpha\,\dfrac{\partial\phi}{\partial x^2}
    \left(%
    -\cot\alpha\,\xhat{e}_1 + \csc\alpha\,\xhat{e}_2
    \right)\\
  &=
    \dfrac{\partial\phi}{\partial x^1}\,\xhat{e}_1
    + \cancelout{\cos\alpha\,\dfrac{\partial\phi}{\partial x^2}\,\xhat{e}_1}
    - \cancelout{\cos\alpha\,\dfrac{\partial\phi}{\partial x^2}\,\xhat{e}_1}
    + \dfrac{\partial\phi}{\partial x^2}\,\xhat{e}_2
\end{align*}
...
\subsection{Métrica en $O'$ desde el punto de vista de $O$}
...
\subsection{Vectores covariantes y contravariantes}
