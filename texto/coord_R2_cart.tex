...
\begin{figure}[ht]
  ...
  \pgfmathsetmacro{\YPLONG}{\EJESLONG}
  % hx, hy
  \pgfmathsetmacro{\LONG}{2.6}
  \pgfmathsetmacro{\HX}{\LONG}
  \pgfmathsetmacro{\HY}{\LONG}
  % Vector A
  \pgfmathsetmacro{\VAMOD}{0.9}
  \pgfmathsetmacro{\VAANG}{30}
  % Vector B
  \pgfmathsetmacro{\VBMOD}{0.9}
  \pgfmathsetmacro{\VBANG}{60}
  % Vector C
  \pgfmathsetmacro{\VCMOD}{0.8}
  \pgfmathsetmacro{\VCANG}{-30}
  % Vector D
  \pgfmathsetmacro{\VDMOD}{1.0}
  \pgfmathsetmacro{\VDANG}{45}  
  % Posición inferior izquierda del elemento de área
  \pgfmathsetmacro{\PX}{0.6}
  ...
  \begin{tikzpicture}[%
    ...
    eje/.style={->},
    proyeccion/.style={black!20, line width=0.4},
    area/.style={fill=green!30, draw=black!20},
    textooriginal/.style={\colorTorig},
    background/.style={%
      line width=\bgborderwidth,
      draw=\bgbordercolor,
...
%    % Proyección de elemento de área en los ejes
    ...
    % Puntos A (inferior) y B (superior)
  ...
  \begin{tikzpicture}[%
    ...
    \path (Pinfdcha) -- coordinate[pos=0.5] (D) (Psupdcha);
    % VECTORES
    \path[save path=\VA] (A) -- ++(\VAANG:\VAMOD) coordinate (VA);
    \path[save path=\VB] (B) -- ++(\VBANG:\VBMOD) coordinate (VB);
    \path[save path=\VC] (C) -- ++(\VCANG:\VCMOD) coordinate (VC);
    \path[save path=\VD] (D) -- ++(\VDANG:\VDMOD) coordinate (VD);
    % COMPONENTES DE LOS VECTORES
    % --- VECTOR VA
    % RECTÁNGULO
    \path[save path=\VArectangulo] (VA|-A) -- (VA) -- (A|-VA);
    % COMPONENTE X
    \path [save path=\VAx] (A) -- (VA|-A) coordinate (VAx);
    % COMPONENTE Y
    \path[save path=\VAy] (A) -- (A|-VA) coordinate (VAy);
    % --- VECTOR VB
    % RECTÁNGULO
    \path[save path=\VBrectangulo] (VB|-B) -- (VB) -- (B|-VB);    
    % COMPONENTE X
    \path [save path=\VBx] (B) -- (VB|-B) coordinate (VBx);
    % COMPONENTE Y
    \path[save path=\VBy] (B) -- (B|-VB) coordinate (VBy);
    % --- VECTOR VC
    % RECTÁNGULO
    \path[save path=\VCrectangulo] (VC|-C) -- (VC) -- (C|-VC);
    % COMPONENTE X
    \path [save path=\VCx] (C) -- (VC|-C) coordinate (VCx);
    % COMPONENTE Y
    \path[save path=\VCy] (C) -- (C|-VC) coordinate (VCy);
    % --- VECTOR VD
    % RECTÁNGULO
    \path[save path=\VDrectangulo]
    (VD|-D) -- (VD) -- (D|-VD);    
    % COMPONENTE X
    \path [save path=\VDx] (D) -- (VD|-D) coordinate (VDx);
    % COMPONENTE Y
    \path[save path=\VDy] (D) -- (D|-VD) coordinate (VDy);
    ...
    % Vectores
    % Vector A
    \draw[vector, use path=\VA];
    \node[above left=-1pt and -8pt] at (VA) {$\vvv{v}_a$};
    % Vector B
    \draw[vector, use path=\VB];
    \node[above left=-1pt and -8pt] at (VB) {$\vvv{v}_{b}$};
    % Vector C
    \draw[vector, use path=\VC];
    \node[below left=-1pt and -8pt] at (VC) {$\vvv{v}_{c}$};
    % Vector D
    \draw[vector, use path=\VD];
    \node[above left=-1pt and -8pt] at (VD) {$\vvv{v}_{d}$};

    % Componentes de vectores
    % Vector A
    \draw[proyeccion, use path=\VArectangulo];
    \draw[faintvectorcomponent, use path=\VAx];
    \draw[redvectorcomponent, use path=\VAy];
    \node[above=-1pt] at (VAy) {$\color{red!80!black}{\vvv{v}_{ay}}$};
    % Vector B
    \draw[proyeccion, use path=\VBrectangulo];
    \draw[faintvectorcomponent, use path=\VBx];
    \draw[redvectorcomponent, use path=\VBy];
    \node[above=-1pt] at (VBy) {$\color{red!80!black}{\vvv{v}_{by}}$};
    % Vector C
    \draw[proyeccion, use path=\VCrectangulo];
    \draw[redvectorcomponent, use path=\VCx];
    \node[right=-2pt] at (VCx) {$\color{red!80!black}{\vvv{v}_{cx}}$};
    \draw[faintvectorcomponent, use path=\VCy];
    % Vector D
    \draw[proyeccion, use path=\VDrectangulo];
    \draw[redvectorcomponent, use path=\VDx];
    \node[right=-2pt] at (VDx) {$\color{red!80!black}{\vvv{v}_{dx}}$};
    \draw[faintvectorcomponent, use path=\VDy];
