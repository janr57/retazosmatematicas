...
\section{Divergencia}
La divergencia de un campo vectorial $\vvv{A}=(A_x, A_y)$ es un escalar
...
será negativa. La divergencia mide la rapidez neta con la que se conduce la materia al
exterior de cada punto, y en el caso de ser la divergencia idénticamente igual a cero,
describe al flujo incompresible del fluido, llamado también campo solenoidal.

\subsection{Demostración}
Supongamos un campo vectorial $\vvv{A} = A_x\,\uvec{\i} + A_y\,\uvec{\j}$, que
podemos escribir también, de una manera \emph{más matemática}\footnotemark{} como
\footnotetext{Porque cuando escribimos $M_x$, por ejemplo, no nos referimos a la
  componente $x$ de $\vvv{A}$, sino a su derivada parcial respecto de $x$,
  es decir, $M_x = \partial M/\partial x$. $M$ de por sí, ya es la componente $x$ del
  campo vectorial.}
\[
  \vvv{A} = M(x,y)\,\uvec{\i} + N(x,y)\,\uvec{\j}
\]

Ahora estamos interesados en investigar una propiedad del campo $\vvv{A}$ en un punto
...
Supongamos, para ello, un pequeño cuadrado imaginario, de lado $h$ en $\symbb{R}^2$,
situando el punto $P$ que nos interesa en el vértice inferior izquierdo, como se aprecia
en la figura
\begin{figure}[ht]
  % Escala
  \def\scl{1}
  ...
  \pgfmathsetmacro{\YPLONG}{\EJESLONG}
  % hx, hy
  \pgfmathsetmacro{\LONG}{2.0}
  \pgfmathsetmacro{\HX}{\LONG}
  \pgfmathsetmacro{\HY}{\LONG}
  % Vector A inferior
  \pgfmathsetmacro{\VAINFMOD}{1.0}
  \pgfmathsetmacro{\VAINFANG}{45}
  % Vector A superior
  \pgfmathsetmacro{\VASUPMOD}{0.9}
  \pgfmathsetmacro{\VASUPANG}{60}
  % Vector B
  \pgfmathsetmacro{\VBIZDOMOD}{0.6}
  \pgfmathsetmacro{\VBIZDOANG}{-30}
  % Vector B'
  \pgfmathsetmacro{\VBDCHOMOD}{0.8}
  \pgfmathsetmacro{\VBDCHOANG}{45}  
  % Posición inferior izquierda del elemento de área
  \pgfmathsetmacro{\PX}{0.9}
...
%    % Proyección de elemento de área en los ejes
    ...
    \path (\PX, \PY) -- coordinate[pos=0.5] (Ainf) (Pinfdcha);
    \path (Psupizda) -- coordinate[pos=0.5] (Asup) (Psupdcha);
    
    % Puntos izquierdo y derecho
    \path (\PX, \PY) -- coordinate[pos=0.5] (Bizdo) (Psupizda);
    \path (Pinfdcha) -- coordinate[pos=0.5] (Bdcho) (Psupdcha);

    % DIBUJOS
    % Proyección del punto (x0,y0) en el eje x
    ...
    % Proyección del punto (x0,y0) en el eje y
    ...
    %\filldraw[area] (P) rectangle coordinate (centro) ++(\HX, \HY) coordinate (Psupdcha);
    ...
    % Puntos inferior A y superior A'
    \fill (Ainf) circle[radius=0.8pt];
    \node[below] at (Ainf) {$A$};
    \fill (Asup) circle[radius=0.8pt];
    \node[above] at (Asup) {$A'$};
    % Puntos iquierdo B y derecho B'
    \fill (Bizdo) circle[radius=0.8pt];
    \node[left] at (Bizdo) {$B$};
    \fill (Bdcho) circle[radius=0.8pt];
    \node[right] at (Bdcho) {$B'$};
  ...
  \begin{tikzpicture}[%
    ...
    faintvectorcomponent/.style={vectorcomponent, black!50},
    ...
    \path[save path=\cuadrado]
    (P) rectangle coordinate (centro) ++(\HX, \HY) coordinate (Psupdcha);
    % PUNTOS INTERMEDIOS A Y A' INFERIOR Y SUPERIOR
    \path (\PX, \PY) -- coordinate[pos=0.5] (Ainf) (Pinfdcha);
    \path (Psupizda) -- coordinate[pos=0.5] (Asup) (Psupdcha);
    % PUNTOS B Y B' IZQUIERDO Y DERECHO
    \path (\PX, \PY) -- coordinate[pos=0.5] (Bizdo) (Psupizda);
    \path (Pinfdcha) -- coordinate[pos=0.5] (Bdcho) (Psupdcha);
    % VECTORES
    \path[save path=\VAinf] (Ainf) -- ++(\VAINFANG:\VAINFMOD) coordinate (VAinf);
    \path[save path=\VAsup] (Asup) -- ++(\VASUPANG:\VASUPMOD) coordinate (VAsup);
    \path[save path=\VBizdo] (Bizdo) -- ++(\VBIZDOANG:\VBIZDOMOD) coordinate
    (VBizdo);
    \path[save path=\VBdcho] (Bdcho) -- ++(\VBDCHOANG:\VBDCHOMOD) coordinate
    (VBdcho);
    % COMPONENTES DE LOS VECTORES
    % --- VECTOR VAINF
    % RECTÁNGULO
    \path[save path=\VAinfrectangulo] (VAinf|-Ainf) -- (VAinf) -- (Ainf|-VAinf);
    % COMPONENTE X
    \path [save path=\VAinfx] (Ainf) -- (VAinf|-Ainf) coordinate (VAinfx);
    % COMPONENTE Y
    \path[save path=\VAinfy] (Ainf) -- (Ainf|-VAinf) coordinate (VAinfy);
    % --- VECTOR VASUP
    % RECTÁNGULO
    \path[save path=\VAsuprectangulo] (VAsup|-Asup) -- (VAsup) -- (Asup|-VAsup);    
    % COMPONENTE X
    \path [save path=\VAsupx] (Asup) -- (VAsup|-Asup) coordinate (VAsupx);
    % COMPONENTE Y
    \path[save path=\VAsupy] (Asup) -- (Asup|-VAsup) coordinate (VAsupy);
    % --- VECTOR VBIZDO
    % RECTÁNGULO
    \path[save path=\VBizdorectangulo] (VBizdo|-Bizdo) -- (VBizdo) -- (Bizdo|-VBizdo);
    % COMPONENTE X
    \path [save path=\VBizdox] (Bizdo) -- (VBizdo|-Bizdo) coordinate (VBizdox);
    % COMPONENTE Y
    \path[save path=\VBizdoy] (Bizdo) -- (Bizdo|-VBizdo) coordinate (VBizdoy);
    % --- VECTOR VBDCHO
    % RECTÁNGULO
    \path[save path=\VBdchorectangulo]
    (VBdcho|-Bdcho) -- (VBdcho) -- (Bdcho|-VBdcho);    
    % COMPONENTE X
    \path [save path=\VBdchox] (Bdcho) -- (VBdcho|-Bdcho) coordinate (VBdchox);
    % COMPONENTE Y
    \path[save path=\VBdchoy] (Bdcho) -- (Bdcho|-VBdcho) coordinate (VBdchoy);
    ...
    % Vectores
    % Vector A inferior
    \draw[vector, use path=\VAinf];
    % Vector A superior
    \draw[vector, use path=\VAsup];
    % Vector B izquierdo
    \draw[vector, use path=\VBizdo];
    % Vector B derecho
    \draw[vector, use path=\VBdcho];

    % Componentes de vectores
    % Vector A inferior
    \draw[proyeccion, use path=\VAinfrectangulo];
    \draw[faintvectorcomponent, use path=\VAinfx];
    \draw[redvectorcomponent, use path=\VAinfy];
    % Vector A superior
    \draw[proyeccion, use path=\VAsuprectangulo];
    \draw[faintvectorcomponent, use path=\VAsupx];
    \draw[redvectorcomponent, use path=\VAsupy];
    % Vector B izquierdo
    \draw[proyeccion, use path=\VBizdorectangulo];
    \draw[redvectorcomponent, use path=\VBizdox];
    \draw[faintvectorcomponent, use path=\VBizdoy];
    % Vector B derecho
    \draw[proyeccion, use path=\VBdchorectangulo];
    \draw[redvectorcomponent, use path=\VBdchox];
    \draw[faintvectorcomponent, use path=\VBdchoy];
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{A la izquierda, pequeño cuadrado de lado $h$ en el punto $P(x_0,y_0)$, en el
    que se han elegido dos puntos arbitrarios en los lados verticales del cuadrado y
    otros dos en los horizontales.}
  %\label{fig:R2-rect-elemento_area}
\end{figure}

Ahora, imaginemos temporalmente que el campo vectorial $\vvv{A}$ es el campo de
velocidades de un fluido, $A(x,y) = v(x,y)$ que se mueve en un plano (con un valor de $z$ constante\footnotemark{}).
...
\[
  \begin{cases}
    \begin{array}{lll}
      ...
      &\longrightarrow
      &M(x_d0+h, y_0+b) h^2 - M(x_0, y_0+b)\,h^2\\[1ex]
      \text{\emph{Caudal} vertical saliente }
      &\longrightarrow
      &N(x_0+da, y_0+b) h^2 - N(x_0+a, y_0+h)\,h^2
    \end{array}
  \end{cases}
\]

Multiplicamos y dividimos entre $h$ y sacamos factor común $h^3$
\[
  \begin{cases}
    \begin{array}{lll}
      \text{\emph{Caudal} horizontal saliente }
      &\longrightarrow
      &Q_x = \dfrac{M(x_0+h, b) - M(x_0, b)}{h}\,h^3\\[1ex]
      \text{\emph{Caudal} vertical saliente }
      &\longrightarrow
      &Q_y = \dfrac{N(a, b) - N(a, y_0+h)}{h}\,h^3
    \end{array}
  \end{cases}
\]
Sumamos los caudales incrementales y pasamos $V=h^3$ al otro miembro.
Supongamos que $M(x,y)$ y $N(x,y)$ son continuas y tienen derivada.
Tomamos el límite $h\to 0$. En estas condiciones $a\to x_0$ y $b\to y_0$
\begin{align*}
  \vvv{\nabla}\cdot\vvv{F}
  &= \lim_{h\to 0}\dfrac{Q}{V}
  = \lim_{h\to 0}\dfrac{Q_x + Q_y}{V}\\
  &= \lim_{h\to 0}\dfrac{M(x_0+h, b) - M(x_0, b)}{h}
  + \lim_{h\to 0}\dfrac{N(a, b) - N(a, y_0+h)}{h} \\
  &= \dfrac{\partial M(x,y)}{\partial x}\Big|_{(x_0,y_0)}
    + \dfrac{\partial N(x,y)}{\partial y}\Big|_{(x_0,y_0)}
