...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    %
    \path (P) -- coordinate[pos=0.5] (hxtext) ++(\HX, 0) coordinate (PHX);
    \path (P) -- coordinate[pos=0.5] (hytext) ++(0, \HY) coordinate (PHY);
    %
%    % Vector
%    \path[save path=\vector] (O) -- coordinate[pos=0.5] (pmid) (\PANG:\PMOD cm)
%    coordinate (P);
%    % Proyección de elemento de área en los ejes
    ...
    \path[save path=\proyPhorx] (PHX) |- coordinate (Phorx) (xfin);
    \path[save path=\proyPverty] (PHY) -| coordinate (Pverty) (yfin);
    ...
    (P) rectangle coordinate (centro) ++(\HX, \HY) coordinate (Pdiag);
    
    % Puntos inferior y superior
    \path (\PX, \PY) -- coordinate[pos=0.5] (Ainf) (PHX);
    \path (PHY) -- coordinate[pos=0.5] (Asup) (Pdiag);
    
    % Puntos izquierdo y derecho
    \path (\PX, \PY) -- coordinate[pos=0.5] (Bizdo) (PHY);
    \path (PHX) -- coordinate[pos=0.5] (Bdcho) (Pdiag);
    ...
    %\filldraw[area] (P) rectangle coordinate (centro) ++(\HX, \HY) coordinate (Pdiag);
  ...
  \begin{tikzpicture}[%
    ...
    area/.style={fill=green!40, draw=black!20},
    textooriginal/.style={\colorTorig},
    vector/.style={-{Latex[round]}, line width=0.4pt},
    faintvectorcomponent/.style={-{Latex[round,width=3pt,length=3pt]}, black!50},
    background/.style={%
      line width=\bgborderwidth,
      draw=\bgbordercolor,
      fill=\bgcolor,
...
%    % Proyección de elemento de área en los ejes
    ...
    % Vectores
    \path[save path=\vectorAinf] (Ainf) -- ++(\VAINFANG:\VAINFMOD) coordinate (VAinf);
    \path[save path=\vectorAsup] (Asup) -- ++(\VASUPANG:\VASUPMOD) coordinate (VAsup);
    \path[save path=\vectorBizdo] (Bizdo) -- ++(\VBIZDOANG:\VBIZDOMOD) coordinate
    (VBizdo);
    \path[save path=\vectorBdcho] (Bdcho) -- ++(\VBDCHOANG:\VBDCHOMOD) coordinate
    (VBdcho);
 
    % Componentes de los vectores
    % Vector VAinf
    \path (VAinf) -- ($(P)!(VAinf)!(PHX)$) coordinate (vainfx);
    \path[save path=\vainfx] (Ainf) -- (vainfx);

    %\fill (PHX) circle[radius=1pt];
    ...
    % Vectores
    % Vector A inferior
    \draw[vector, use path=\vectorAinf];
    % Vector A superior
    \draw[vector, use path=\vectorAsup];
    % Vector B izquierdo
    \draw[vector, use path=\vectorBizdo];
    % Vector B derecho
    \draw[vector, use path=\vectorBdcho];
    ...
    % \draw[red] (VAinf) -- (vainfx);

    %\fill[red] (VAinf) circle[radius=2pt];
