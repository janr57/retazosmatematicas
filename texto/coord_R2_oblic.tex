...
\begin{enumerate}
\item Analizar el sistema de coordenadas oblicuas en el plano euclídeo $\symbb{R}^2$,
  utilizando unos conceptos básicos de geometría diferencial.
\item Realizar algunas operaciones diferenciales, como gradiente, divergencia y
  laplaciano en este sistema.
\end{enumerate}

Se utilizarán determinados conceptos de la geometría diferencial, como
\emph{base natural}, \emph{métrica}, \emph{factores de escala},
...
matemáticas. Por descontado, la geometría diferencial es mucho más, pues solo se ha
arañado una pequeña parte de la superficie.
...
\label{sect:R2-oblic_vision_general}
\begin{itemize}
\item Ejes y su notación:
  ...
  \item Notación en álgebra lineal y física clásica:
    El eje horizontal se suele representar como $x$, $\tilde{x}$, $x'$ u otro símbolo, y
    el     inclinado como $y$, $\tilde{y}$, $y'$ u otro símbolo.
    La notación con tilde o prima se suele usar para diferenciar visualmente dos
    sistemas cartesianos, como el ortogonal estándar y el oblicuo, o entre dos oblicuos.
    
    En este texto, a menos que se diga lo contrario, cuando utilicemos esta notación con
    tilde o prima, reservaremos $x$ e $y$ para el sistema cartesiano ortogonal, aunque
    el sistema oblicuo no lo comparemos con aquél.
  \item Notación en cálculo tensorial y geometría diferencial:
    Para los ejes, se usan preferentemente superíndices $x^1$ y $x^2$, u otros
    (en el caso de superíndices numéricos, estos no representan exponentes).
    
    Se debe aclarar que también se representarán con superíndice las componentes
    contravariantes en este sistema; en todo caso, si son ejes o componentes se
    distinguirán por el contexto\footnotemark{}).
    \footnotetext{En el sistema cartesiano ortogonal se hace algo parecido, los ejes se
      suelen representar como $x$ e $y$, y las componentes de los vectores también se
      pueden representar igual, distinguiéndose por el contexto.}
  \end{itemize}
\item Base natural:
  ...
  \[
    B = \set{\vvv{e}_1, \vvv{e}_2}
  \]
  Los vectores se han representado sin acento circunflejo porque aunque en este sistema
  son vectores unitarios, como veremos más tarde,  esto no es cierto en otros sistemas
  de coordenadas.
  
  Avanzamos un detalle importante: los vectores de la base covariante $\vvv{e}_i$
  (con subíndice) son los que sirven para medir las componentes contravariantes $x^i$
  (con superíndice) de cualquier otro vector, como se aprecia en la figura
  \ref{fig:R2-oblic_contrav}.
\end{itemize}
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
  ...
  Por último, en esta figura se considera que el eje $\tilde{y}$ se ha formado
  inclinando el eje $y$ un  ángulo en sentido horario $\beta = \pi/2 - \alpha$, aunque
  podría hacerlo en sentido horario, formando un ángulo obtuso con el eje de abcisas.}
  \label{fig:R2-oblic_ejes}
\end{figure}

\section{Bases unitaria y natural}

\subsection{Base unitaria}
En física general se suelen utilizar bases formadas por vectores unitarios, aunque para
formar una base no es necesario que sean unitarios, basta con que conste del máximo
número de vectores linealmente independientes. Así, cualquier vector del espacio se podrá
representar como una combinación lineal de vectores de la base.

Vamos obtener la base unitaria $B'_u=\set{\xhat{u}_1, \xhat{u}_2}$ en el sistema de
coordenadas oblicuas. Si nos fijamos en la figura \ref{fig:R2-oblic_u1u2}, podemos
expresar estos vectores en función de los $\uvec{\i}$ y $\uvec{\j}$ de las cartesianas
rectangulares
\begin{align}
  \label{eq:R2-oblic_u1_ij}
  \xhat{u}_1
  &=
    |\xhat{u}_1|\,\uvec{\i} + 0\,\uvec{\j} = \uvec{\i}\\
  \label{eq:R2-oblic_u2_ij}
  \xhat{u}_2
  &=
    |\xhat{u}_2|\cos{\alpha}\, \uvec{\i} + |\xhat{u}_2|\sin{\alpha}\, \uvec{\j}
    = \cos\alpha\,\uvec{\i} + \sin\alpha\,\uvec{\j}
\end{align}

%Ahora vamos a obtener una base $B'=\set{\xhat{e}'_1, \xhat{e}'_2}$ para el sistema
%oblicuo, obligando de antemano a que sus vectores sean unitarios, $|\vvv{e}'_i| = 1$.
%Estos vectores unitarios se suelen utilizar en las bases de los sistemas de coordenadas
%en física general. Pero, cuando se utiliza esta restricción, no siempre se obtiene la
%base natural del sistema.
%
%Para relacionar esta base con la estándar de las rectilíneas
%$B=\set{\uvec{\i},\uvec{\j}}$ haremos coincidir el versor $\{e}'_1$ con $\uvec{\i}$,
%esto es, el eje $\tilde{x}$ coincidirá con el eje $x$ de las cartesianas. Véase la figura
%\ref{fig:R2-oblic_u1u2}.
\begin{figure}[ht]
  \centering
  % Escala
  \def\scl{1.00}
  ...
  \begin{tikzpicture}[%
    ...
    ++(\VYTILDEANG:\VTILDEMOD cm) coordinate (u2fin);
    % Proyección de P en los ejes
    % Con eje x
    \path[save path=\proyx] (u2fin) |- (xfin) coordinate[pos=0.5] (u2x) (xfin);
    % Posición de texto en eje x
    \path (O) -- coordinate[pos=0.5] (uxtxt) (u2x);
    % Con eje y
    \path[save path=\proyy] (u2fin) -| coordinate[pos=0.5] (u2y) (yfin);
    % Posición de texto en eje y
    \path (O) -- coordinate[pos=0.5] (uytxt) (u2y);
    ...
    \node[below] at (uxtxt) {\footnotesize $\cos\alpha$};
    \draw[proyeccion, use path=\proyy];
    \node[rotate=90, below left=4pt and 11pt] at (u2y) {\footnotesize $\sin\alpha$};
  ...
  \caption{Sistema de coordenadas cartesianas oblicuas $\tilde{x}$ e $\tilde{y}$, que
    ...
    rojo los vectores unitarios $\xhat{u}_1$ y $\xhat{u}_2$ del sistema oblicuo y sus
    proyecciones en los ejex $x$ e $y$.}
  \label{fig:R2-oblic_u1u2}
\end{figure}

\subsection{Base natural}
Como se comentó en \ref{sect:R2-oblic_vision_general}, la base natural $B'$ en el sistema
oblicuo (con prima, para distinguirla temporalmente de la base del sistema de coordenadas cartesianas ortogonal $B=\set{\uvec{\i},\,\uvec{\j}}$), está formada por los vectores
covariantes (con subíndice) siguientes
\begin{equation}\label{eq:R2-oblic_base_natural}
  B' = \set{\vvv{e}_1, \vvv{e}_2}
\end{equation}
que nos podrá servir para obtener la métrica de estos sistemas oblicuos que, a su vez,
nos permitirá relacionar las componentes covariantes y las contravariantes, entre sí.

\subsubsection{Cálculo geométrico}
Hasta que no obtengamos la llamada \emph{matriz de cambio de base}, para obtener la base
natural vamos recurrir a un procedimiento geométrico.

Los vectores de la  base natural se calculan derivando parcialmente el vector de
posición de un punto genérico del plano en función de las componentes contravariantes
$x^1$ y $x^2$ en el sistema oblicuo.
\begin{equation}
  \vvv{e}_i = \dfrac{\partial\vvv{r}}{\partial x^i}
  \,\text{;}\hspace{0.8em}
  \text{con } i = 1, 2.
\end{equation}
Hasta que no se explique con algo de detalle el significado de estas componentes
contravariantes, basta con fijarnos en coordenadas cartesianas ortogonales en la figura \ref{fig:R2-coord_rect-ij}.
De acuerdo con ella, el vector de posición de un punto cualquiera del plano se expresa
como
\begin{equation}
  \label{eq:R2-oblic_vector_posicion_cartesianas}
  \vvv{r} = x\,\uvec{\i} + y\,\uvec{\j}
\end{equation}

Nuestro objetivo es obtener $x$ e $y$ en función de las componentes contravariantes en oblicuas, $x^1$ y $x^2$
\begin{align}
  x &= x(x^1, x^2)\\
  y &= y(x^1, x^2)
\end{align}
y sustituir en la expresión \eqref{eq:R2-oblic_vector_posicion_cartesianas}, para obtener
el vector de posición en función de las componentes contravariantes $x^i$.

Fijémonos en el triángulo sombreado en verde de la figura
\ref{fig:R2-oblic_proc_geom_agudo}, donde hemos supuesto que los ejes forman un ángulo
agudo.
Sabemos que el ángulo $\beta$ es el complementario de $\alpha$, que es el que forman los
dos ejes del sistema oblicuo, de manera que $\sin\beta = \cos\alpha$ y
$\cos\beta = \sin\alpha$.

Así
\begin{equation}
  \label{eq:R2-oblic_transf_agudo}
  \begin{cases}
    \begin{array}{lll}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    % Ángulo alfa
    \path (xfin) -- (O) -- (ytildefin) pic
    [angulo, "\color{black}{\scriptsize $\alpha$}", angle radius=16pt,
  ...
  \caption{El triángulo rectángulo sombreado en verde nos permite relacionar las
    ...
    del ángulo $\beta$ que forma el eje oblicuo $x^2$ con el eje ortogonal $y$.}
  \label{fig:R2-oblic_proc_geom_agudo}
\end{figure}
...
Derivando parcialmente, los vectores de la base natural serán
\begin{align}
  \vvv{e}_1
  &=
    ...
    = \uvec{\i}
    \longrightarrow
    |\vvv{e}_1| = |\uvec{\i}| = \sqrt{1} = 1\\
  \vvv{e}_2
  &=
    \dfrac{\partial\vvv{r}}{x^2}
    = \cos\alpha\,\uvec{\i} + \sin\alpha\,\uvec{\j}
    \longrightarrow
    |\vvv{e}_2| = \sqrt{\cos^2\alpha + \sin^2\alpha} = 1
\end{align}
y deducimos que sus módulos valen la unidad.

Además, podemos comprobar que coinciden con los vectores unitarios $\xhat{u}_i$ que
obtuvimos gráficamente en \eqref{eq:R2-oblic_u1_ij} y \eqref{eq:R2-oblic_u2_ij}
\begin{align}
  \label{eq:R2-oblic_e1_ij}
  \vvv{e}_1
  &=
    \xhat{u}_1
    = \uvec{\i}
    = (1, 0)\\
  \label{eq:R2-oblic_e2_ij}
  \vvv{e}_2
  &=
    \xhat{u}_2
    = \cos\alpha\,\uvec{\i} + \sin\alpha\,\uvec{\j}
...
\subsubsection{¿Hubiera dado lo mismo si el ángulo $\alpha$ entre los ejes
  hubiera sido mayor de \qty{90}{\degree}?}
Demostraremos que la base natural sería la misma que la que hemos calculado suponiendo
que los ejes oblicuos formaban un ángulo agudo. Fijémonos en la figura
\ref{fig:R2-oblic_proc_geom_obtuso}, que representa esta situación.
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
  % 
  \pgfmathsetmacro{\EJECARTLONG}{4.5}
  \pgfmathsetmacro{\EJEOBLICLONG}{4.0}
  % Eje x
  \pgfmathsetmacro{\XANG}{0.0}
  \pgfmathsetmacro{\XLONG}{\EJECARTLONG}
  % Eje y
  \pgfmathsetmacro{\YANG}{90}
  \pgfmathsetmacro{\YLONG}{0.85*\EJECARTLONG}
  % Eje xtilde
  \pgfmathsetmacro{\XTILDEANG}{0}
  \pgfmathsetmacro{\XTILDELONG}{\EJEOBLICLONG}    
  % Eje ytilde
  \pgfmathsetmacro{\YTILDEANG}{90+30}
  \pgfmathsetmacro{\YTILDELONG}{0.45*(\EJECARTLONG + \EJEOBLICLONG)}
  % Vector P
  \pgfmathsetmacro{\PMOD}{3.0}
  \pgfmathsetmacro{\PANG}{30}
  % Distancia horizontal entre P e ytilde
  \pgfmathsetmacro{\Px}{\PMOD * cos(\PANG)}
  \pgfmathsetmacro{\Py}{\PMOD * sin(\PANG)}
  \pgfmathsetmacro{\YTILDEx}{\YTILDELONG * cos(\YTILDEANG)}
  % Fondo
  \pgfmathsetmacro{\HORZ}{0.25}
  \pgfmathsetmacro{\VERT}{0.25}
  %
 ...
 \begin{tikzpicture}[%
    ...
    textoapagado/.style={\colorTorig},
    vector/.style={-{Latex[round]}, shorten >=1.2pt, line width=.8pt,\colorV},
    contravariante/.style={line width=2.0pt, red},
    cartesiana/.style={gray, line width=1.5pt},
    contravtxt/.style={red!70!black},
    pcirculo/.style={fill=\colorPorig, draw=black},
    angulo/.style={fill=green!15, draw=\colorGirodos},
    ...
    \path[save path=\proyy] (P) -| (yfin) coordinate[pos=0.5] (py);
    % Componentes contravariantes
    % Intersección de la horizontal desde P hasta ytilde
    % -> Componente contravariante eje ytilde (CTV2) 
    \path[name path=horizontal] (\YTILDEx, \Py) -- (P);
    \path [name intersections={of=O--ytilde and horizontal, by=CTV2}];
    ...
    % delta x
    \draw[red!50!black, line width=0.3pt, use path=\deltaxsegmento];
    \node[below] at (deltaxtxt) {$x^1-x$};
    % y
    %\draw[cartesiana, use path=\ysegmento];
    %\node[right] at (ytxt) {$y$};
    \path (px) -- node[pos=0.41, left] {$y$} (P);
    % x^2
    \path (CTV1) -- node[pos=0.43, right] {$x^2$} (P);
  ...
  \label{fig:R2-oblic_proc_geom_obtuso}
\end{figure}

Utilizamos de nuevo el vector de posición de un punto arbitrario del plano en
función de las componentes cartesianas
\[
  \vvv{r} = x\,\uvec{\i} + y\,\uvec{\j}
\]

Sabemos que $\alpha = \pi/2 + \beta$ y por tanto,
\begin{align*}
  \sin\alpha &= \sin(\pi/2 + \beta) = \cos\beta\\
  \cos\alpha &= \cos(\pi/2 + \beta) = -\sin\beta
\end{align*}

Ahora calculamos las coordenadas $x$ e $y$ en función de las contravariantes $x^1$ y
$x^2$, fijándonos en el triángulo sombreado en verde
\begin{equation}
  \begin{cases}
    \begin{array}{lll}
      \sin\beta = -\cos\alpha = \dfrac{x^1-x}{x^2}
      &\longrightarrow
      &x = x^1 + x^2\cos\alpha\\
      \cos\beta = \sin\alpha = \dfrac{y}{x^2}
      &\longrightarrow
...
Como vemos, la transformación entre $(x,y)$ y $(x^1,x^2)$ es idéntica a la que se obtuvo
en \eqref{eq:R2-oblic_transf_agudo}, por tanto, dicha transformación es independiente del
ángulo $0<\alpha<\pi$.
Podemos concluir que la matriz de cambio de base será única.

\section{Cambio de base}
Las expresiones \eqref{eq:R2-oblic_e1_ij} y \eqref{eq:R2-oblic_e2_ij} se pueden
se pueden representar matricialmente
\begin{equation}
  \begin{pmatrix}
    \vvv{e}_1 &  \vvv{e}_2
  \end{pmatrix}
  ...
  \end{pmatrix}
\end{equation}
Obsérvese que los vectores de las bases se representan en filas.

Así, la matriz que permite obtener la nueva base $B'$ a partir de la antigua $B$, está
formada por las componentes de los vectores $(1,0)$ y $(\cos\alpha,\sin\alpha)$
obtenidos en las expresiones \eqref{eq:R2-oblic_e1_ij} y \eqref{eq:R2-oblic_e2_ij},
...
Como dijimos, la matriz de cambio de base de $B$ a $B'$ está formada por las componentes
...
base oblicua $B'$. Al contrario, para obtener la base rectangular a partir de la oblicua,
utilizaríamos la matriz inversa $\mmm{M}^{-1}$
\begin{subequations}
\begin{align}
  &\begin{pmatrix}
    \vvv{e}_1 &  \vvv{e}_2
...
\emph{es más pequeña} que la antigua (centímetros en lugar de metros); pero para que el
vector $\vvv{v}$ no cambie, los valores medidos en centímetros deben \emph{aumentar} con
respecto a las componentes en metros ($A'_i > A_i$)
\[
...
``se han hecho 100 veces más grandes''.
...
Así, un vector como el que hemos puesto de ejemplo en \eqref{eq:R2-oblic_componentes}, se
podría representar genéricamente como
\[
...
segmento orientado $\vvv{v}$. Esta es la forma natural con la que sumamos vectores
mediante la regla del paralelogramo, ver figura \ref{fig:R2-oblic_contrav}.
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
  % 
 ...
 \begin{tikzpicture}[%
    ...
    % Proyección de P en los ejes
    % Con eje x
    \path[save path=\proyx] (u2fin) |- (xfin) coordinate[pos=0.5] (e2x) (xfin);
    % Componentes contravariantes
    % Intersección de horizontal desde P hasta ytilde
    % -> Componente contravariante eje ytilde (CTV2) 
    \path[name path=P--y] (P) -| (yfin);
    ...
    % Marcar en color la componentes contravariantes
    \draw[contravariante]
    (O) -- node[contravtxt,pos=0.5, below] {$x^1\vvv{e}_1$} (CTV1);
  ...
  \caption{Componentes contravariantes $x^i$, en coordenadas oblicuas. El vector
    ...
    $\vvv{v}=x^1\,\vvv{e}_1+x^2\,\vvv{e}_2$. La componente
    contravariante en un cierto eje nos indica el desplazamiento que se ha producido en
    ese eje, en las unidades en las que se mide el vector $\vvv{v}$, según la base
    covariante elegida.}
...
en el eje $x^1$, medido por el vector $\vvv{e}_1$ de la base, y $x^2$ nos lo da en el
eje $x^2$, medido por $\vvv{e}_2$.
La suma de las componentes contravariantes $x^i$ multiplicadas por sus
respectivos vectores covariantes de la base $\vvv{e}_i$, nos da el desplazamiento
...
\[
  ...
  \,\begin{pmatrix}
    ...
    x^1
  \end{pmatrix}
  = \vvv{e}_1 x^1 + \vvv{e}_2 x^2
  = x^1 \vvv{e}_1 + x^2 \vvv{e}_2
  = \vvv{e}_i x^i
  = x^i \vvv{e}_i
\]
...
\[
  \vvv{v}
  = \tilde{x}^1\,\vvv{e}_1 + \tilde{x}^2\,\vvv{e}_2 = \tilde{x}^i\,\vvv{e}_i
  = \left(2+\dfrac{4}{\sqrt{3}}\right)\,\vvv{e}_1 - \dfrac{8}{\sqrt{3}}\,\vvv{e}_2
\]
...
\section{Métrica}
Con los conocimientos que hemos adquirido, la forma más directa y rápida para obtener el
tensor métrico en coordenadas oblicuas es utilizar el método seguido en la expresión
\eqref{eq:R2-cart-metrica}, para el sistema cartesiano rectangular. Para ello debemos
calcular el producto escalar de los vectores de la base \eqref{eq:R2-oblic_base_natural}.
Cualquier otro método es un rodeo para no utilizar esto.

Multiplicamos escalarmente los vectores \eqref{eq:R2-oblic_e1_ij} y
\eqref{eq:R2-oblic_e2_ij} de la base natural entre sí
\begin{subequations}
  \begin{align}
    ...
    &=
      \vvv{e}_1\cdot\vvv{e}_2 = \cos\theta\\
...
\item Los elementos no diagonales no son cero, lo que indica que un cambio cualquiera en
  ...
  \item Un caso extremo posible se produciría cuando
    ...
    coordenadas oblicuo, sino un sistema de coordenadas cartesianas rectangulares, como
  ...
  \item El otro caso extremo, que no produciría ningún sistema de referencia en
    ...
    unitarios respectivos, serían iguales $\vvv{e}_1 = \vvv{e}_2 = \uvec{\i}$ y ya no
...
\item Covectores o formas lineales (covariantes): Son elementos del espacio dual $V^*$
  asociado a $V$. Son funciones $\omega$ que actúan sobre los vectores $\vvv{v}\in V$ y
  producen un escalar $\mathbb{F}$. Sus componentes se transforman de la misma forma que
  los cambios de base (matriz de cambio de base $\mmm{M}$.
  
  A este nivel, las componentes covariantes son simplemente los coeficientes de una
  \emph{1-forma} o un funcional lineal ---las funciones que operan sobre un solo
  vector--- y producen un escalar $\mathbb{F}$, asociado al espacio vectorial.
  Sus componentes en nuestro sistema oblicuo en $\symbb{R}^2$ se representan
  matricialmente como
  \[
    \omega
    = \begin{pmatrix}
      ...
      \end{pmatrix}
  \]
  Estos covectores \emph{viven} en un espacio dual $V^*$ asociado al espacio vectorial
  $V$ original, $\omega\in V^*$
  \[
    \begin{array}{llcl}
      \omega: & V & \longrightarrow & \symbb{F}\\[-0.5ex]
      & \scriptstyle\vvv{v} & \mapsto & \scriptstyle\omega(\vvv{v})
    \end{array}
  \]
  En los casos que nos ocupan en este texto, el campo escalar es el de los números reales $\mathbb{F} = \mathbb{R}$ y el escalar que se obtiene en nuestro caso es el cuadrado del módulo del vector
  \[
    \omega(\vvv{v}) = |\vvv{v}|^2 = v^2
  \]
...
\end{itemize}
 En principio, esta es la relación entre los vectores y los covectores,
 pues \emph{viven} en espacios distintos. Ahora bien, si disponemos de una métrica (un
 producto escalar definido en $V$, también lo tendremos en $V^*$), ambos espacios se
 pueden conectar y el álgebra lineal nos permite \emph{bajar} o \emph{subir} índices
 convirtiendo un vector en un covector y viceversa.
...
\eqref{eq:R2-oblic_base_natural}, pues contiene un superíndice (de nuevo, no es un exponente).
...
\begin{figure}[ht]
  \begin{minipage}{0.45\linewidth}  
    ...
    \begin{tikzpicture}[%
      ...
      % Proyección de P en los ejes
      % Con eje x
      \path[save path=\proyx] (P) |- (xfin) coordinate[pos=0.5] (e2x) (xfin);
      % Componente covariante con eje xtilde (COV1)
      \path (P) |- coordinate[pos=0.5] (COV1) (xtildefin);
      % Perpendicular a O--ytilde que pasa por P
      % -> Componente covariante con eje ytilde (COV2)
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      % Con eje y
      \path[save path=\proyy] (P) -| coordinate[pos=0.5] (e2y) (yfin);
      % Posición de texto en eje y
...
espacio, la base unitaria $B= (\vvv{e}_1,\vvv{e}_2)$---
\begin{equation}
  \phi = \phi(x^1, x^2)
\end{equation}
...
\begin{equation}
  \vvv{\nabla}\phi
  = g^{ij}(\nabla\phi)_i\,\vvv{e}_j
  = g^{ij} \dfrac{\partial\phi}{\partial x^i}\,\vvv{e}_j
  = (\nabla\phi)^j\,\vvv{e}_j
\end{equation}
...
\begin{align*}
  \vvv{\nabla}\phi
  &=
    \mmm{g}^{11}\dfrac{\partial\phi}{\partial x^1}\,\vvv{e}_1
    + \mmm{g}^{12}\dfrac{\partial\phi}{\partial x^1}\,\vvv{e}_2
    + \mmm{g}^{21}\dfrac{\partial\phi}{\partial x^2}\,\vvv{e}_1
    + \mmm{g}^{22}\dfrac{\partial\phi}{\partial x^2}\,\vvv{e}_2\\
  &=
    \dfrac{1}{\sin^2\alpha}\,\dfrac{\partial\phi}{\partial x^1}\,\vvv{e}_1
    - \dfrac{\cos\alpha}{\sin^2\alpha}\,\dfrac{\partial\phi}{\partial x^1}\,\vvv{e}_2
    - \dfrac{\cos\alpha}{\sin^2\alpha}\,\dfrac{\partial\phi}{\partial x^2}\,\vvv{e}_1
    + \dfrac{1}{\sin^2\alpha}\,\dfrac{\partial\phi}{\partial x^2}\,\vvv{e}_2
\end{align*}
