...
En física general se suelen utilizar bases formadas por vectores unitarios, aunque
formar una base no es necesario que sean unitarios, basta con que conste del máximo
número de vectores linealmente independientes. Así, como sabemos, cualquier vector del
espacio se podrá representar como una combinación lineal de vectores de la base.
...
Como se comentó en el apartado anterior, la base natural $B'$ en el sistema
oblicuo (con prima, para distinguirla temporalmente de la base ortogonal
$B=\set{\uvec{\i},\,\uvec{\j}}$), está formada por los vectores covariantes
(con subíndice) siguientes
...
Los vectores de la  base natural se calculan derivadando parcialmente el vector de
...
contravariantes, basta con fijarnos en la figura \ref{fig:R2-coord_rect-ij}.
...
en \ref{eq:R2-oblic_transf_agudo}, por tanto, dicha transformación es independiente del
ángulo $0<\alpha<\pi$.
...
\begin{equation}
  ...
  \end{pmatrix}
\end{equation}
Obsérvese que los vectores de la base se representan en filas.
...
vectores de la base \eqref{eq:R2-oblic_base_natural}. Cualquier otro método es un rodeo
para no utilizar esto.
