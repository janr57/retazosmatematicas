...
\begin{enumerate}
\item Analizar el sistema de coordenadas oblicuas en el plano $\symbb{R}^2$ y la
  expresión del resultado de distintas operaciones en este sistema.
...
\item Ejes y su notación:
  ...
  \end{itemize}
\item Base:
  La base más utilizada comúnmente en física es la formada por los vectores unitarios
  radial $\xhat{u}_r = \xhat{r}$ y angular $\xhat{u}_\theta = \xhat{\theta}$.
  ...
  contravariantes $x^i$ de los vectores, como se aprecia en la figura
  \ref{fig:R2-oblic_contrav}.

  La base natural servirá para obtener la métrica de estos sistemas oblicuos.
\end{itemize}
\begin{figure}[ht]
  % Escala
  \def\scl{1.00}
...
Como se comentó en el apartado anterior, la base natural $B'$ en el sistema oblicuo está
formada por los dos vectores covariantes (nótese el subíndice en los vectores)
\begin{equation}\label{eq:R2-oblic_base}
  B' = \set{\xhat{e}_1, \xhat{e}_2}
\end{equation}

\subsubsection{Cálculo}
Tomaremos la figura \ref{fig:R2-coord_rect-ij} como referencia.
