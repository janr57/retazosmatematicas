...
\item Introducir algunas ideas de la geometría diferencial, como \emph{base natural},
  \emph{métrica}, etc., que serán más importantes en otros sistemas de coordenadas.
...
%    \end{scope}
  \end{tikzpicture}
  \caption{Coordenadas cartesianas rectangulares de un punto $P$ del plano (en verde) y
    vectores unitarios (en rojo). Se puede comprobar que el vector de posición del punto
...
%la figura \ref{fig:R2-rect_ij}, respecto de cada una de ellas. Estos vectores
...
%Esta sección se encuentra aquí de una manera un tanto forzada, pues los dos tipos de
%vectores del título son el mismo en coordenadas rectangulares y, como consecuencia, aquí
%no es necesario hacer referencia a ellos.
%
%En todo caso, adelantaremos algo de información sobre ellos para captar su significado y
%entender por qué en otros sistemas de coordenadas estos vectores son diferentes, al
%contrario de lo que ocurre en este sistema de coordenadas.
%
%\subsection{Vectores contravariantes}
%Los vectores \emph{viven} en un espacio vectorial $V$\, llamado \emph{espacio tangente}.
%Pensemos, por ejemplo, en un espacio curvo como puede ser el de una superficie esférica.
%No se pueden representar vectores en una superficie curva, sino en un plano. Ese plano
%es en este caso el espacio tangente o espacio vectorial donde \emph{viven} los vectores.
%
...
%    
%    % 1. ESFERA
%    \shade[ball color=black!20!white, opacity=\OPACIDADESFERA] (C) circle[radius=\R cm];
...
%%  \def\scl{1.9}
%%  % Origen de coordenadas
%%  \pgfmathsetmacro{\Ou}{0.0}
%%  \pgfmathsetmacro{\Ov}{0.0}
%%  % Centro de la esfera
%%  \pgfmathsetmacro{\Cu}{\Ou}
%%  \pgfmathsetmacro{\Cv}{\Ov}
%%  % Radio de la esfera en cm
%%  \pgfmathsetmacro{\R}{1.0}
%%  % Opacidad de la esfera y los planos
%%  \pgfmathsetmacro{\OPACIDADESFERA}{0.9}
%%  \pgfmathsetmacro{\OPACIDADPLANO}{0.8}
%%  % Punto de vista
%%  \pgfmathsetmacro{\THETA}{90-30}
%%  \pgfmathsetmacro{\PHI}{40}
%%  % Eje x
%%  \pgfmathsetmacro{\EJEXu}{-sin(\PHI)}
%%  \pgfmathsetmacro{\EJEXv}{-cos(\THETA)*cos(\PHI)}
%%  % Eje y
%%  \pgfmathsetmacro{\EJEYu}{cos(\PHI)}
%%  \pgfmathsetmacro{\EJEYv}{-cos(\THETA)*sin(\PHI)}
%%  % Eje z (punto P1 (Pa))
%%  \pgfmathsetmacro{\EJEZu}{0}
%%  \pgfmathsetmacro{\EJEZv}{sin(\THETA)}
%%  % Punto P2 (Pb)
%%  \pgfmathsetmacro{\Pbtheta}{45}
%%  \pgfmathsetmacro{\Pbphi}{50}
%%  \pgfmathsetmacro{\Pbx}{sin(\Pbphi)*cos(\Pbphi)}
%%  \pgfmathsetmacro{\Pby}{sin(\Pbphi)*sin(\Pbphi)}
%%  \pgfmathsetmacro{\Pbz}{cos(\Pbphi)}
%%  \pgfmathsetmacro{\Pbu}{-\Pbx*sin(\PHI)+\Pby*cos(\PHI)}
%%  \pgfmathsetmacro{\Pbv}
%%  {-\Pbx*cos(\THETA)*cos(\PHI)-\Pby*cos(\THETA)*sin(\PHI)+\Pbz*sin(\THETA)}
%%  %
%%  \centering
%%  \begin{tikzpicture}[%
%%    scale=\scl,
...
%%    % COORDENADAS
%%    % Origen
%%    \coordinate (O) at (\Ou,\Ov);
%%    % Centro de la esfera
%%    \coordinate (C) at (\Cu,\Cv);
%%    % Eje x
%%    \path[save path=\ejex] (C) -- (\EJEXu, \EJEXv);
%%    % Eje y
%%    \path[save path=\ejey] (C) -- (\EJEYu, \EJEYv);
%%    % Eje z y P1
%%    \path[save path=\ejez] (C) -- (\EJEZu, \EJEZv) coordinate (P1);
%%    % P2
%%    coordinate (P2) at (\Pbu, \Pbv);
%%    
%%    % 1. ESFERA
%%    \shade[ball color=black!20!white, opacity=\OPACIDADESFERA] (C) circle[radius=\R cm];
%%    \draw[gray!10] (C) circle[radius=\R cm];
%%
%%%    % PRUEBA. EJES DE COORDENADAS
%%%    \draw[->, use path=\ejex];
%%%    \draw[->, use path=\ejey];
%%%    \draw[->, use path=\ejez];
%%    
%%%   % 2. PLANO SUPERIOR
%%
%%    % 3. PUNTO SUPERIOR Y PLANO
%%    \fill[black] (P1) circle[radius=0.5pt];
%%
%%    % 4. PUNTO P2
%%    \fill[black] (P2) circle[radius=0.5pt];
%%    
%%    %    % 2. PLANO SUPERIOR
%%%    \coordinate (P1) at (xyz spherical cs:radius=1,latitude=90);
%%%    \begin{scope}[canvas is xy plane at z=1]
...
%  \pgfmathsetmacro{\OPACIDADPLANO}{0.8}
%  % Punto de vista
%  \pgfmathsetmacro{\THETA}{90.0-30.0}
%  \pgfmathsetmacro{\PHI}{40.0}
%  % Eje x
%  
%  % Eje y
%  
%  % Eje z (punto P1 (Pa))
%  %\pgfmathsetmacro{\EJEZu}{\proyEjeZu{\THETA}{\PHI}}
%  %\pgfmathsetmacro{\EJEZv}{\proyEjeZv{\THETA}{\PHI}}
%  % Punto P2 (Pb)
%  \pgfmathsetmacro{\Pbtheta}{45}
%  \pgfmathsetmacro{\Pbphi}{50}
%  % 
...
%    %\path[save path=\ejey] (C) -- (\EJEYu, \EJEYv);
%    % Eje z y P1
%    %\coordinate (ejez) at \ejeZmac;
%    %\path[save path=\ejez] (C) -- (ejez);
%    % P2
%    %\coordinate (P2) at (\Pbu, \Pbv);
%
%    % 1. ESFERA
...
%
%    \fill[red] \puntoPolar{5}{math.pi/4} circle (1pt);
%    %\draw[blue, thick] (0,0) -- \puntoPolar{3}{math.pi/6} node[right] {Extremo};
%    % PRUEB. EJES DE COORDENADAS
%%    \draw[->, use path=\ejex];
%%    \draw[->, use path=\ejey];
%    %\draw[->, use path=\ejez];
%    %\node at (O) {$\proyEjeZu{\THETA}{\PHI}$};
%    %\fill[red] (ejez) circle[radius=1pt];
%    
%    %\node at (O) {\ejeZmac};
%    %\def\novale{\proyEjeZu{\THETA}{\PHI}};
%    %\node at (O) {$\novale$};
%    %\fill[red] (ejez) circle[radius=2pt];
%    %\draw[->,red] (0,0,0) -- (0,0,1);
%%   % 2. PLANO SUPERIOR
%
%%    % 3. PUNTO SUPERIOR Y PLANO
%%    \fill[black] (P1) circle[radius=0.5pt];
%%    \node at (O) {\pruebaTexto};
%
%%    % 4. PUNTO P2
%%    \fill[black] (P2) circle[radius=0.5pt];
%    
