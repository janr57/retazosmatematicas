...
\item Introducir algunas ideas de geometría diferencial, tales como
  \emph{base natural}, \emph{métrica}, etc., que serán más importantes en otros
  sistemas de coordenadas.
\item Recordar la expresión del resultado de unas pocas operaciones de cálculo en
  este sistema.
\end{enumerate}

\section{Ideas básicas}
El vector de posición de un punto del espacio $\symbb{R}^2$ en coordenadas cartesianas
es:
\begin{equation}\label{eq:R2-r_rect}
  \vvv{r}
  = x\,\uvec{\i} + y\,\uvec{\j}
\end{equation}
En la figura \ref{fig:R2-rect_ij} lo representamos, junto con los vectores unitarios
del sistema:
\begin{figure}[ht]
...
Los vectores de la base natural se obtienen derivando parcialmente el vector de
...
\begin{subequations}
  \begin{align}\label{eq:R2-cart-ex}
    ...
    &=
      ...
      = \dfrac{\partial}{\partial x} (x \uvec{i} + y \uvec{\j})
      = \uvec{\i}
      = \xhat{u}_x
    ...
    &= \dfrac{\partial\vvv{r}}{\partial y}
      = \dfrac{\partial}{\partial y} (x \uvec{i} + y \uvec{\j})
      = \uvec{\j}
      = \xhat{u}_y
      \hspace{1em}\longrightarrow\hspace{1em}
    |\vvv{e}_y| = |\uvec{\j}| = 1
...
Así, la \emph{base natural} en cartesianas rectangulares, y que coincide
...
\section{Métrica}
La métrica en un sistema de coordenadas dado define la estructura del espacio, y
en particular, sin ella no se pueden medir distancias ni ángulos. Es como si
proporciona un sistema de reglas y goniómetros con los cuales medir distancias
y ángulos.
...
La base natural, en coordenadas cartesianas en este espacio, está formada por
los vectores $\uvec{\i}$ y $\uvec{\j}$, en los ejes $x$ e $y$, respectivamente,
ver figura \ref{fig:R2-rect_ij}.
...
La métrica en cualquier sistema de coordenadas se calcula a través del producto escalar
entre los vectores de la base natura. En nuestro caso, el resultado es la matriz
unidad:
...
La raíz cuadrada positiva de los elementos diagonales de la métrica se llaman
...
\begin{itemize}
\item Los elementos no diagonales son nulos. Esto se debe a que los vectores
  $\vvv{e}_x$ y $\vvv{e}_y$ son independientes, esto es, ortogonales, lo que
  implica que el cambio en una coordenada no afectará a la otra.
  Así, si nos desplazamos una distancia cualquiera en el eje $x$, no habremos
  recorrido ninguna en el eje $y$, y viceversa. Así, $h_{xy}= h_{yx}= 0$.
\item Los elementos diagonales valen la unidad. Esto implica que un cambio en
  una coordenada, implica una distancia recorrida igual en el espacio.
...
vector $\vvv{v}$ se mide realizando el producto escalar del vector por sí mismo.
El producto escalar como se estudia en física y matemáticas elementales sirve para
la coordenadas cartesianas rectangulares, aunque se puede ampliar para otros sistemas,
utilizando la métrica y el cálculo matricial.
En cartesianas, y utilizando la métrica da el resultado familiar:
\[
  \vvv{v}\cdot\vvv{w}
...
Esta sección se presenta de una manera un tanto forzada, pues los dos tipos de vectores
del título son el mismo en coordenadas rectangulares y no es necesario hacer referencia a
ellos.

Pero, servirá para adquirir algo de intuición acerca de ellos, donde en otros sistemas de
coordenadas estos vectores son diferentes, al contrario de lo que ocurre en este.
...
\footnotetext{Para poder realizar este tipo de operaciones entre dos vectores que se
  ...
  \emph{transporte paralelo}, pero esto es algo que no nos debe preocupar por ahora.}
\begin{figure}[ht]
  % Escala
  \def\scl{1.9}
...
% ********************************************************************************
% Código lua de prueba para representar planos y vectores en superficie esférica.

\begin{luacode*}
  -- 0. Escala y unidad de longitud
  escala=1.0
  unidad="cm"
  
-- 1. Datos y propiedades de la esfera:
  esf = {
    radio=2.0,color="gray!10",opac=1,sombracolor="black!20",sombraopac=0.9
  }
  
  -- 2, Posición angular del observador
  obs = {thetaD = 65, phiD = 15}
  --obs = {thetaD = 90, phiD = 0}

  -- 3. Propiedades de los puntos en la esfera
  ptpr = {
    fsize="scriptsize", invisib=true, rv=0.025, ri=0.015, colorv="black", colori="red",
    opacv=1.0, opaci=0.5
  }
  
  -- 3. Matriz de puntos (tabla de tablas). Algunos pueden no ser visibles:
  ptos = {
    {
      thetaD=90, phiD=186, a=0.8, b=0.8, opac=0.8, draw="black!80", fill="black!10",
      p="$P_1$", pos="below left"
    },
    {
      thetaD=0, phiD=0, a=2.0, b=2.0, opac=0.8, draw="blue!80!black", fill="blue!20",
      p="$P_2$", pos="below left=-2pt and -2pt"
    },
    {
      thetaD=90, phiD=170, a=2, b=2, opac=0.8, draw="green!80!black", fill="green!20",
      p="$P_3$", pos="above left"
    },
    {
      thetaD=70, phiD=50, a=2.0, b=2.0, opac=0.8, draw="red!80!black", fill="red!20",
      p="$P_4$", pos="above left=-2pt and -2pt"
    }
  }

  -- 4. Matriz de planos tangente. Se crea la matriz de los planos:
  planos = {}
  
  -- 5. Propiedades de los vectores en el plano tangente:
  -- lw: grosor del segmento;
  -- width, length: anchura y longitud de la punta de la flecha.
  vecpr = {
    width="3.5pt", length="3.5pt", lw="0.7pt"
  }

  -- 6. Coordenadas 2D de los vectores en el plano tangente:
  vectores = {
    {planoID=1, r=0.9, thetaD=45},
    {planoID=1, r=0.9, thetaD=0},
    {planoID=2, r=0.7, thetaD=90},
    {planoID=2, r=0.7, thetaD=0},
    {planoID=4, r=0.5, thetaD=0},
    {planoID=4, r=0.75, thetaD=240}
  }
  
  -- 7. Dibujo
  -- Añade u, v a la tabla ptos.
  -- Cada punto se representa en una pantalla plana (papel, ordenador, etc.)
  -- (u,v) son las coordenadas en esta pantalla.
  -- Las dos funciones siguientes calculan las coordenadas u, v del punto y los cuatro
  -- puntos representativos de los planos.
  mEsfera.completaPuntos(unidad, esf, obs, ptos)
  mEsfera.completaPlanos(unidad, esf, obs, ptos, planos)
  mEsfera.completaVectores(unidad, esf, obs, ptos, planos, vectores)
  
\end{luacode*}

% El siguiente bloque imprime los valores calculados (para depuración).
{\scriptsize
  \par
  \noindent\DEBUGEsf{escala}{unidad}{esf}\par
  \noindent\DEBUGObs{obs}\par
  \noindent\DEBUGPuntosProp{ptpr}\par\vspace{2ex}
  \noindent\DEBUGCoordPuntos{ptos}\par
  \noindent\DEBUGPlanos{planos}\par
  \noindent\DEBUGPlanosObs{planos}\par
  \noindent\DEBUGVectores{vectores}\par
}

\begin{figure}[ht]
  % El primer parámetro es la escala del entorno TikZpicture
  \TIKZEsferaPlanos{escala}{unidad}{esf}{obs}{ptpr}{ptos}{planos}{vecpr}{vectores}
\end{figure}

\section{Gradiente}
Presentamos un resumen del gradiente, tal y como se presenta en un curso básico de
cálculo.
...
indica la dirección y sentido en la que el campo $\phi$ aumenta más rápidamente. Su
módulo representa el ritmo de variación del campo $\phi$ en la dirección de dicho vector
gradiente.
...
\end{equation}
Mide la diferencia entre el flujo saliente y el flujo entrante de un
campo vectorial sobre la superficie que rodea a un volumen de control. Si el campo tiene
\emph{fuentes} la divergencia será positiva, y si tiene \emph{sumideros} será negativa.

Este operador mide la rapidez neta con la que se conduce el campo al exterior de cada
punto, y en el caso de ser igual a cero, se trataría del flujo incompresible del campo,
llamado también campo solenoidal.
...
%\]
%
%Estamos interesados en investigar una propiedad del campo $\vvv{F}$ en un punto concreto
%$P(x_0,y_0)$ del espacio $\symbb{R}^2$. En particular, estamos interesados en estudiar si
%ese punto es una fuente del campo (punto en el que se crean líneas de campo), si es un
%sumidero (si desaparecen líneas), o si no ocurre ninguna de las dos cosas (las líneas de
%campo entran y salen del punto, sin más).
%
...
%    background/.style={%
%      line width=\backgroudborderwidth,
%      draw=\backgroundbordercolor,
%      fill=\backgroundcolor,
%    },
%    ]
...
%    redvectorcomponent/.style={vectorcomponent, red},
%    background/.style={%
%      line width=\backgroundborderwidth,
%      draw=\backgroundbordercolor,
%      fill=\backgroundcolor,
%    },
%    ]
