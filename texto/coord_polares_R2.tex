...
\subsection{Vectores unitarios y su producto escalar}
La base estándar en coordenadas cartesianas en este espacio está formada por los vectores $\uvec{\i}$ y $\uvec{\j}$, en los ejes $x$ e $y$, respectivamente.
...
\subsection{Vectores unitarios, sus productos escalares y derivadas}
\subsubsection{Vectores unitarios}
Representaremos a los vectores unitarios en coordenadas polares como $\xhat{r}$ y
$\xhat{\theta}$.
Nos proponemos relacionarlos con los $\uvec{\i}$ y $\uvec{\j}$ de las cartesianas.
Empecemos por el vector unitario radial
$\xhat{r}$. De la figura \ref{fig:R2:coordenadas-R2}, inferimos que el vector
$\vvv{r}$ se puede expresar como
\begin{equation}
  \vvv{r} = r\cos\theta\,\uvec{\i} + r\sin\theta\,\uvec{\j}
\end{equation}
El vector unitario radial se obtiene dividiendo el vector entre su módulo
\begin{equation}
  \xhat{r}
  = \dfrac{\vvv{r}}{r}
  = \cos\theta\,\uvec{\i} + \sin\theta\,\uvec{\j}
\end{equation}
El vector unitario angular es la derivada de $\xhat{r}$ con respecto de
$\xhat{\theta}$
\begin{equation}
  \xhat{\theta}
...
\end{equation}
Se puede comprobar fácilmente que $\xhat{r}$ y $\xhat{\theta}$ son ortogonales, pues
su producto escalar es cero: $\xhat{r}\cdot\xhat{\theta} = 0$.
Resumimos este cambio de base y su inversa de forma matricial
\begin{subequations}
\begin{align}
  \begin{pmatrix}
    \xhat{r}\\
...
\end{align}
\end{subequations}
donde se ha utilizado la inversa de la matriz de cambio de base, que por ser una matriz
ortogonal, la inversa es su transpuesta.
\subsubsection{Productos escalares}
\begin{align}
  \xhat{r}\cdot\xhat{r}
  &= (\cos\theta\,\uvec{\i}+\sin\theta\,\uvec{i})
    \cdot (\cos\theta\,\uvec{\i}+\sin\theta\,\uvec{i})
    = \cos^2\theta + \sin^2\theta
    = 1\\
  \xhat{r}\cdot\xhat{\theta}
  &= (\cos\theta\,\uvec{\i}+\sin\theta\,\uvec{i})
    \cdot (-\sin\theta\,\uvec{\i}+\cos\theta\,\uvec{i})
    = -\sin\theta \cos\theta + \sin\theta\cos\theta
    = 0\\
\end{align}
donde hemos utilizado el producto escalar de los vectores unitarios en cartesianas, ver
ecuaciones~\eqref{eq:R2-ii-producto escalar}, \eqref{eq:R2-ij-producto escalar},
...
con respecto de las coordenadas polares
\begin{subequations}
\begin{align}
  \dfrac{d\xhat{r}}{dr}
  &= 0\\
...
\subsection{Métrica y factores de escala}
Vamos a calcular la métrica y los factores de escala de varias formas equivalentes:
\begin{enumerate}
  \item Primero la calculamos utilizando las
