...
Se llaman \emph{factores de escalas} o \emph{coeficientes métricos} a la raíz cuadrada de
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    % Proyección de P en los ejes    
%    \node[textooriginal,above=5pt] at (OPmidway) {$\vvv{r}$};
%    \filldraw[pcirculo] (P) circle [radius=1.4pt];
%    \node[textooriginal,above] at (P) {$P$};
%    % Punto P'
%    \draw[vectorrotado] (O) -- (P');
%    \node[textorotado,right=0pt] at (OP'midway) {$\vvv{r}'$};
%    \filldraw[pprimacirculo] (P') circle [radius=1.4pt];
%    \node[right] at (P') {$P'$};
%    % Sentido de giro del vector
%    \draw [-{Latex},\colorGiro,shorten <= 3pt]
%    (parrow) to[bend left=30] (p'arrow);
%    % Incremento x
%    \draw[pcirculo] (P |- O) circle[radius=1.5pt];
%    \filldraw[pprimacirculo] (P' |- O) circle[radius=1.5pt];
