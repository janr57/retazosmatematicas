...
\begin{figure}[ht]
  ...
  \caption{Coordenadas cartesianas de un punto $P$ del plano y vectores unitarios.
    De esta figura, comprobamos que $\vvv{r} = x\uvec{i} + y\uvec{j}$.}
...
\begin{subequations}
  ...
  \end{align}
\end{subequations}
La base así obtenida la denominamos \emph{base natural}. Estos son importantes para
calcular la métrica en cartesianas
\begin{equation}\label{eq:R2-basefundamental}
  B=\set{\vvv{e}_x, \vvv{e}_y} = \set{\uvec{\i},\, \uvec{\j}}
\end{equation}
Hacemos notar que los vectores de esta base natural, en este sistema de coordenadas, son
los vectores unitarios tradicionales en este sistemas, $\uvec{\i}$ y $\uvec{\j}$.
De aquí se puede inferir que la métrica en coordenadas cartesianas rectangulares en
$\symbb{R}^2$ se representará como una matriz unidad (en el apartado
\ref{ssect:R2-metrica-factores_escala} se calculará con detalle).
...
El concepto de producto escalar se puede ampliar mediante cálculo matricial, utilizando
...
coordenadas, $r$ y $\theta$. La primera coordenada es la distancia de un punto del espacio al origen de coordenadas y un punto del espacio, y la segunda es el ángulo que
forma el eje de abcisas con la línea que une el origen de coordenadas con el punto, ver
figura \ref{fig:R2-coordenadas-R2}.
