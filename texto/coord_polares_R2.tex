...
\item Enfoque matricial.
  ...
  %\hspace{1em}
  El problema se debe a que, aunque hemos utilizado la métrica, esta no es lo único que
  cambia cuando pasas de cartesianas a polares, esto es, no solo cambian las distancias
  (la métrica), sino también cambian los vectores de la base con la posición en el plano.
  En este cálculo matricial, y en el que se realizó en cartesianas, está implícito el que
  los vectores de la base son inmutables, lo que es correcto en cartesianas pero no en
  polares, dado que los vectores cambian con la posición en el plano $\symbb{R}^2$.
...
\item Enfoque basado en los vectores unitarios.
