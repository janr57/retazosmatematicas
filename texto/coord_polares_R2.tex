...
\item En este producto escalar se utiliza la métrica en polares
  ...
  El fallo se debe a que hemos utilizado la métrica, y esto solo corrige el factor de
  escala en coordenadas polares. Pero en este cálculo matricial y en el que se realizó en
  cartesianas, se ha supuesto implícitamente que los vectores de la base son inmutables,
  lo que es correcto en cartesianas pero, en polares, los vectores cambian con la
  posición ($\xhat{\theta}$ depende de la posición en el plano $\symbb{R}^2$).
  El sistema de coordenadas polares es curvilíneo y esa curvatura hay que tenerla en
  cuenta, ver figura \ref{fig:R2-coordenadas-polares}.
  \begin{figure}[ht]
  % Escala
  \def\scl{1}
  %
  % Número de círcunferencias
  \pgfmathsetmacro{\NUMCIRC}{4} 
  ...
  \begin{tikzpicture}[%
    ...
    pcirculo/.style={fill=\colorPorig, draw=black},
    angulogirado/.style={fill=\colorAngGreen, draw=\colorGirodos},
    polares/.style={draw=black!30, line width=0.5pt},
    background/.style={%
      line width=\bgborderwidth,
      draw=\bgbordercolor,
      fill=\bgcolor,
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Coordenadas polares (curvilíneas) $r$ y $\theta$, donde $r$ representa la
    distancia al origen (polo) y $\theta$ es el ángulo que forma con el eje $x$, junto
    con los vectores unidad $\xhat{r}$ y $\xhat{\theta}$ respectivos, en distintas
    posiciones del plano $\symbb{R}^2$.}
