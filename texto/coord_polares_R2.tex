...
\item Los elementos no diagonales son nulos. Esto se interpreta como que los vectores
  ...
  Esto es, un cambio en una coordenada no afecta a la otra. Así, si nos desplazamos
  una distancia cualquiera en el eje $x$, no habremos recorrido ninguna distancia en
  el eje $y$, y viceversa, por lo que $h_{xy}=h_{yx}=0$.
\item Los elementos diagonales valen la unidad. Cada elemento diagonal de la métrica que
  ...
  cualquier distancia, digamos un metro, es fácil entender que en espacio nos habremos
...
\begin{enumerate}
\item Calculamos directamente la métrica utilizando los productos escalares
